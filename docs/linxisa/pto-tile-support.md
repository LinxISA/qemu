...
TCONCAT TEXTRACT TINSERT TIMG2COL TCI TTRI
THISTOGRAM TQUANT TDEQUANT TSORT TMRGSORT TGATHER TSCATTER
```

PTO 0.58.5 retires `TFILLPAD`, `TTRANS`, `TPARTADD`, `TPARTMUL`, `TPARTMAX`,
and `TPARTMIN`. Their former selector values are reserved and QEMU rejects
them before tile execution.
