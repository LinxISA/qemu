...
int linx_socket_hart_count(const MachineState *ms, int socket_id)
{
    ...
    first_hartid = linx_socket_first_hartid(ms, socket_id);
    if (first_hartid < 0) {
        return -1;
    }

    last_hartid = linx_socket_last_hartid(ms, socket_id);
    if (last_hartid < 0) {
        return -1;
    }
...
void linx_socket_fdt_write_distance_matrix(const MachineState *ms, void *fdt)
{
    ...
    if (numa_enabled(ms) && ms->numa_state->have_numa_distance) {
        dist_matrix_size = linx_socket_count(ms) * linx_socket_count(ms);
        dist_matrix_size *= (3 * sizeof(uint32_t));
        dist_matrix = g_malloc0(dist_matrix_size);

        for (i = 0; i < linx_socket_count(ms); i++) {
            for (j = 0; j < linx_socket_count(ms); j++) {
                idx = (i * linx_socket_count(ms) + j) * 3;
