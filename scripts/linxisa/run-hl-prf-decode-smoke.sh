...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/linxisa-hl-prf-decode.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT

SRC="$ROOT/tests/linxisa/hl_prf_decode_smoke.s"
OUT_O="$TMP/hl_prf_decode_smoke.o"
