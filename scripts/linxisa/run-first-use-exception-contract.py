...
def run_qemu(qemu: Path, image: Path, *, stop_after_trace: bool) -> tuple[int, str]:
    env = os.environ.copy()
    env.update(
        {
    ...
    while time.monotonic() < deadline and process.poll() is None:
        for key, _ in selector.select(timeout=0.05):
            ...
            if not line:
                continue
            output.append(line)
            if stop_after_trace and line.startswith(TRACE_PREFIX):
                process.terminate()
                break
        if stop_after_trace and any(line.startswith(TRACE_PREFIX) for line in output):
            break
...
def assert_first_use(line: str, kind: int) -> None:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="linx-first-use-") as directory:
        ...
        for test_case, name in enumerate(VECTOR_HEADERS + CUBE_HEADERS):
            ...
            _, output = run_qemu(args.qemu, image, stop_after_trace=True)
        ...
        for name, test_case, retry_kind in (
            ...
            returncode, output = run_qemu(args.qemu, image, stop_after_trace=False)
            if returncode != 0:
