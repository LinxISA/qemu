...
static bool linx_tile_output_reuses_bound_source(const CPULinxState *env,
                                                 unsigned last_index,
                                                 unsigned tile)
{
    for (unsigned i = 0; i < last_index && i < LINX_TILE_MAX_IOT; i++) {
        for (unsigned source = 0; source < 2; source++) {
            if ((env->tile_iot_src_valid[i] & (1u << source)) != 0u &&
                env->tile_iot_src_phys[i][source] == tile) {
                return true;
            }
        }
    }
...
static bool linx_tile_reserve_output(
    const CPULinxState *env,
    const uint16_t live[LINX_TILE_HAND_COUNT],
    const uint16_t reserved[LINX_TILE_HAND_COUNT],
    const uint8_t order[LINX_TILE_HAND_COUNT][LINX_TILE_HAND_DEPTH],
    const uint8_t count_by_hand[LINX_TILE_HAND_COUNT],
    const LinxTileIOTDesc *desc, unsigned output_index,
    unsigned *tile_out)
{
    unsigned hand = 0;
    ...
    for (unsigned depth = 0; depth < LINX_TILE_HAND_DEPTH; depth++) {
        if ((live[hand] | reserved[hand]) & LINX_TILE_HAND_BIT(depth)) {
            continue;
        }
    ...
    /* A push ages Q#16 out of the architectural producer-age window. */
    if (count_by_hand[hand] == LINX_TILE_HAND_DEPTH) {
        const unsigned oldest = order[hand][LINX_TILE_HAND_DEPTH - 1u];
        const unsigned depth = oldest % LINX_TILE_HAND_DEPTH;
        const bool oldest_hand_ok = oldest / LINX_TILE_HAND_DEPTH == hand;
        const bool oldest_live =
            (live[hand] & LINX_TILE_HAND_BIT(depth)) != 0u;
        const bool oldest_reserved =
            (reserved[hand] & LINX_TILE_HAND_BIT(depth)) != 0u;
        const bool oldest_pinned = env->tile_pin_owner[oldest] != 0u;
        const bool source_reuse =
            linx_tile_output_reuses_bound_source(env, output_index, oldest);

        if (oldest_hand_ok && oldest_live && !oldest_reserved &&
            !oldest_pinned && !source_reuse) {
            *tile_out = oldest;
            return true;
        }
        if (linx_debug_local_enabled_p()) {
            qemu_log_mask(LOG_GUEST_ERROR,
                          "Linx tile reserve full hand=%u count=%u"
                          " oldest=%u q16_pin=0x%x live=0x%04x"
                          " reserved=0x%04x output_index=%u\n",
                          hand, count_by_hand[hand], oldest,
                          env->tile_pin_owner[oldest], live[hand],
                          reserved[hand], output_index);
            qemu_log_mask(LOG_GUEST_ERROR,
                          "Linx tile reserve reasons hand_ok=%u live=%u"
                          " reserved=%u pin=%u source=%u\n",
                          oldest_hand_ok, oldest_live, oldest_reserved,
                          oldest_pinned, source_reuse);
        }
    }
    return false;
...
/* Apply one already-bound output to a private producer-age window copy. */
static bool linx_tile_stage_output(
    ...
    uint8_t count_by_hand[LINX_TILE_HAND_COUNT], unsigned tile)
{
    ...
    if (count_by_hand[hand] == LINX_TILE_HAND_DEPTH) {
        const unsigned oldest = order[hand][LINX_TILE_HAND_DEPTH - 1u];
        const unsigned oldest_depth = oldest % LINX_TILE_HAND_DEPTH;

        /* A full window can only reuse the architectural Q#16 slot. */
        if (oldest / LINX_TILE_HAND_DEPTH != hand || oldest != tile) {
            return false;
        }
        live[hand] &= ~LINX_TILE_HAND_BIT(oldest_depth);
        order[hand][LINX_TILE_HAND_DEPTH - 1u] = 0u;
        count_by_hand[hand]--;
    } else if ((live[hand] & LINX_TILE_HAND_BIT(depth)) != 0u) {
        return false;
    }
...
static bool linx_tile_group_fp32_sizes(const CPULinxState *env,
                                       uint32_t *left_bytes,
                                       uint32_t *right_bytes,
                                       uint32_t *output_bytes,
                                       unsigned *output_size_code)
{
    const uint64_t m = env->lb[0];
    const uint64_t n = env->lb[1];
    const uint64_t k = env->lb[2];
    const uint64_t left = m * k * sizeof(uint32_t);
    const uint64_t right = k * n * sizeof(uint32_t);
    const uint64_t output = m * n * sizeof(uint32_t);
    unsigned size_code;

    if (m == 0u || n == 0u || k == 0u || left > 8192u ||
        right > 32768u || output > 8192u ||
        !linx_tile_size_code_from_bytes((uint32_t)output, &size_code)) {
        return false;
    }
    *left_bytes = left;
    *right_bytes = right;
    *output_bytes = output;
    *output_size_code = size_code;
    return true;
}
    ...
    const CPULinxState *env, LinxTMATransferDir dir)
{
    ...
    return linx_tile_decode_tma_format(env->tile_arg_format, dir);
}
                                                   ...
                                                   bool chain)
{
    ...
    /* Validate the restored return target before publishing SP/RA. */
    if (!linx_bstart_cache_fast_hit(env, value)) {
        HELPER(linx_check_bstart_target)(env, value);
    }
                                        ...
                                        bool chain)
{
    ...
    if (restore_count > 0) {
        ...
        /* Validate the restored return target before publishing frame state. */
        if (!linx_bstart_cache_fast_hit(env, restored_ra)) {
            HELPER(linx_check_bstart_target)(env, restored_ra);
        }
    }
                                       ...
                                       bool chain)
{
    ...
    const int restore_count =
        linx_frame_restore_prepare(env, stacksize, new_sp, restore_base,
                                   ...
                                   &restore_host_loads,
                                   &restore_fallback_loads);
    /* FRET.RA resolves its target from pre-restore RA. */
...
static uint64_t linx_tile_tepl_binary_qword(CPULinxState *env, uint32_t op,
...
static bool linx_tile_tepl_binary_qword_checked(
...
static bool linx_tile_tepl_binary_word_checked(
                                ...
                                uint32_t physical_cols, uint32_t bytes)
{
    ...
    if (!linx_tile_set_shape(env, dst_tile, mask_words_per_row, rows,
                             mask_words_per_row, rows)) {
        return false;
    }
    return true;
}

static bool linx_tile_tepl_tcmps(CPULinxState *env, unsigned dst_tile,
                                 ...
                                 uint32_t physical_cols, uint32_t bytes)
{
    ...
    linx_tile_set_elem_bytes(env, dst_tile, sizeof(uint32_t));
    linx_tile_set_dtype(env, dst_tile, 25u); /* packed predicate is U32 */
    return linx_tile_set_shape(env, dst_tile, mask_words_per_row, rows,
                               mask_words_per_row, rows);
}

static bool linx_tile_tepl_select(CPULinxState *env, unsigned dst_tile,
                                  ...
                                  uint32_t cols, uint32_t physical_cols)
{
    ...
    if ((env->tile_reg_dtype[mask_tile] & 0x1fu) != 25u ||
        env->tile_reg_elem_bytes[mask_tile] != sizeof(uint32_t) ||
        env->tile_reg_valid_cols[mask_tile] < mask_words_per_row ||
        env->tile_reg_valid_rows[mask_tile] < rows ||
        mask_physical_cols < mask_words_per_row ||
        env->tile_reg_rows[mask_tile] < rows ||
        (uint64_t)rows * mask_physical_cols * sizeof(uint32_t) >
    ...
    for (uint32_t r = 0; r < rows; r++) {
        for (uint32_t c = 0; c < cols; c++) {
            ...
            const uint32_t mask_lane =
                r * mask_physical_cols + c / 32u;
            uint32_t mask_word = 0;
            uint64_t value = scalar;

            if (!linx_tile_get_elem(env, mask_tile, mask_lane,
                                    sizeof(uint32_t), &mask_word)) {
                return false;
            }
            if ((mask_word & (1u << (c & 31u))) != 0u) {
...
static uint64_t linx_tile_tepl_one(uint32_t dtype)
{
    switch (dtype & 0x1fu) {
    case 0u: /* FP64 */
...
/*
 * Row-reduce destination stride: the destination allocation holds
 * `valid_rows x stride` elements (TROWSUM.md: Col/LB2 is the row-major
 * stride of the reduced tile; the compiler sizes the destination TSize as
 * rows x stride x elem).  A dense 1-column vector is the stride-1 special
 * case.  Returns false when the allocation cannot hold an integral number
 * of rows at any stride.
 */
static inline bool linx_tile_tepl_row_reduce_dst_stride(
    const CPULinxState *env, uint32_t bytes64, unsigned dst_elem_bytes,
    unsigned *stride_out)
{
    const uint64_t rows = (uint32_t)(env->lb[1] & 0xffffu);

    if (rows == 0u || dst_elem_bytes == 0u || bytes64 == 0u ||
        bytes64 % (rows * dst_elem_bytes) != 0u) {
        return false;
    }
    const uint64_t stride = bytes64 / (rows * dst_elem_bytes);
    if (stride == 0u || stride > UINT32_MAX) {
        return false;
    }
    *stride_out = (unsigned)stride;
    return true;
}
                                   ...
                                   uint32_t physical_cols, uint32_t bytes)
{
    ...
    unsigned dst_stride = 1u;
    if (row_reduce &&
        !linx_tile_tepl_row_reduce_dst_stride(env, bytes, elem_bytes,
                                              &dst_stride)) {
        return false;
    }
    for (uint32_t output = 0; output < output_count; output++) {
        ...
        if (!linx_tile_set_elem(env, dst_tile, output * dst_stride,
                                elem_bytes, product)) {
            return false;
        }
    }
    ...
    if (row_reduce) {
        return linx_tile_set_shape(env, dst_tile, 1u, rows, dst_stride,
                                   bytes / (dst_stride * elem_bytes));
    }
                                      ...
                                      uint32_t bytes)
{
    ...
    unsigned dst_stride = 1u;
    if (row_reduce &&
        !linx_tile_tepl_row_reduce_dst_stride(env, bytes, sizeof(uint32_t),
                                              &dst_stride)) {
        return false;
    }
    for (uint32_t output = 0; output < output_count; output++) {
    ...
    if (row_reduce) {
        return linx_tile_set_shape(env, dst_tile, 1u, rows, dst_stride,
                                   bytes / (dst_stride * sizeof(uint32_t)));
    }
...
static bool linx_tile_tepl_expand(CPULinxState *env, unsigned dst_tile,
                                  unsigned src0_tile, unsigned src1_tile,
...
static bool linx_tile_tepl_partial_binary(CPULinxState *env,
                                          unsigned dst_tile,
                                          unsigned src0_tile,
                                          unsigned src1_tile,
                                          unsigned binary_op,
    ...
    for (uint32_t r = 0; r < dst_rows; r++) {
        for (uint32_t c = 0; c < dst_cols; c++) {
            ...
            if (src0_valid && src1_valid) {
                result = linx_tile_tepl_binary_word(env, binary_op, dtype,
                                                    src0, src1);
            } else if (src0_valid) {
                result = src0;
            } else if (src1_valid) {
...
static bool linx_tile_tepl_concat(CPULinxState *env, unsigned dst_tile,
                                  unsigned src0_tile, unsigned src1_tile,
                                  uint32_t rows, uint32_t cols,
                                  uint32_t physical_cols)
{
...
static bool linx_tile_tepl_gatherb(CPULinxState *env, unsigned dst_tile,
...
static bool linx_tile_tepl_extract(CPULinxState *env, unsigned dst_tile,
...
static bool linx_tile_tepl_dequant(CPULinxState *env, unsigned dst_tile,
...
static inline uint32_t linx_tile_tepl_unary_word(CPULinxState *env,
                                                 uint32_t op, uint32_t dtype,
                                                 uint32_t value)
{
    const unsigned elem_bytes = linx_tile_dtype_elem_bytes(dtype);
...
static bool linx_tile_tepl_shape(const CPULinxState *env, unsigned elem_bytes,
                               ...
                               unsigned size_code)
{
    ...
    if ((op != 0x0c7u && op != 0x0c8u) ||
        ...
        !linx_tile_tepl_shape(env, sizeof(uint32_t),
...
/*
 * The execution helpers below predate the 0.57.1 Mode+Function allocation.
 ...
 */
static uint32_t linx_tile_tepl_impl_selector(uint32_t selector)
{
    switch (selector) {
...
static bool linx_tile_tepl_impl_selector_executable(uint32_t op)
{
    switch (op) {
    case 0x000u: /* TADD */
    case 0x001u: /* TSUB */
...
static bool linx_tile_tepl_impl_dtype_supported(uint32_t op, uint32_t dtype)
{
    const uint32_t dt = dtype & 0x1fu;
    ...
    case 0x034u: /* TSELS */
        ...
        break;
    case 0x02du: /* TABS */
        /* TABS.md: dtypes are only F32/F16. */
        supported = fp32 | fp16;
        break;
    case 0x02fu: /* TNEG */
...
static int linx_tile_tepl_impl_source_arity(uint32_t op)
{
    switch (op) {
    case 0x019u: /* TEXPANDS */
    case 0x080u: /* TCI */
    ...
    case 0x107u: /* TMRGSORT */
        return 2;
    default:
        return linx_tile_tepl_impl_selector_executable(op) ? 2 : -1;
    }
}

static bool linx_tile_tepl_dtype_supported(uint32_t selector, uint32_t dtype)
{
    const uint32_t impl = linx_tile_tepl_impl_selector(selector);

    return impl != UINT32_MAX &&
           linx_tile_tepl_impl_dtype_supported(impl, dtype);
}

static uint64_t linx_tile_tepl_unary_qword(CPULinxState *env, uint32_t op,
...
static uint64_t linx_tile_tepl_convert64(CPULinxState *env, uint64_t value,
                                          uint32_t src_dtype,
                                          unsigned src_bytes,
                                          uint32_t dst_dtype,
                                          unsigned dst_bytes)
...
static bool linx_tile_tepl64_core(CPULinxState *env, unsigned dst_tile,
                                  ...
                                  uint32_t bytes)
{
    ...
    if (op == 0x02bu) {
        return source_count == 2u &&
               linx_tile_tepl_tcmp(env, dst_tile, src0, src1, rows, cols,
                                    physical_cols, bytes);
    }
    if (op == 0x033u) {
        return source_count == 1u && has_scalar &&
               linx_tile_tepl_tcmps(env, dst_tile, src0, scalar, rows, cols,
                                     physical_cols, bytes);
    }
    if (op == 0x00du) {
        ...
        for (uint32_t i = 0; i < active; i++) {
            ...
            if (!linx_tile_get_elem64(env, src0, lane, src_bytes, &value) ||
                !linx_tile_set_elem64(
                    ...
                    linx_tile_tepl_convert64(env, value, src_dtype, src_bytes,
        ...
        for (uint32_t o = 0; o < outer; o++) {
            ...
            for (uint32_t i = 0; i < inner; i++) {
                ...
                if (!linx_tile_get_elem64(env, src0, lane, 8u, &value))
                    return false;
                result = linx_tile_tepl_binary_qword(
                    env, sum ? 0x000u : (maximum ? 0x004u : 0x005u),
                    dtype, result, value);
            }
            unsigned dst_stride = 1u;
            if (row_reduce &&
                !linx_tile_tepl_row_reduce_dst_stride(env, bytes, 8u,
                                                       &dst_stride)) {
                return false;
            }
            if (!linx_tile_set_elem64(env, dst_tile,
                                      o * dst_stride, 8u, result))
                return false;
        }
        return true;
    }
    ...
    for (uint32_t i = 0; i < active; i++) {
        ...
        if (source_count > 1u) {
            ...
            result = linx_tile_tepl_unary_qword(env, op, dtype, lhs);
        }
...
static bool linx_tile_tepl64_core_op(uint32_t op)
{
    return op <= 0x00bu || (op >= 0x00du && op <= 0x019u) ||
           (op >= 0x020u && op <= 0x02fu && op != 0x02cu) || op == 0x030u ||
           op == 0x032u || op == 0x033u || op == 0x080u;
}
...
static inline uint32_t linx_tile_effective_dtype(const CPULinxState *env)
{
    ...
    if ((env->tile_attr_dtype & 0x100u) != 0u &&
        (allowed & LINX_DATR_DATA_TYPE) != 0u) {
        return env->tile_attr_dtype & 0x1fu;
    }
...
static inline unsigned linx_tile_tepl_row_reduce_output_elems(
    const CPULinxState *env)
{
    return (unsigned)(env->lb[1] & 0xffffu);
}

static inline bool linx_tile_tepl_row_reduce_dst_legal(
    const CPULinxState *env, uint32_t impl_op, uint32_t bytes64,
    unsigned dtype_elem_bytes)
{
    const unsigned dst_elem_bytes =
        linx_tile_tepl_impl_is_arg_reduce(impl_op) ? 4u : dtype_elem_bytes;
    const unsigned output_elems = linx_tile_tepl_row_reduce_output_elems(env);
...
static bool linx_tile_tepl_row_reduce_shape(const CPULinxState *env,
                                            unsigned elem_bytes,
                                            uint32_t elems,
                                            uint32_t *rows_out,
                                            uint32_t *cols_out,
...
static bool linx_tile_tepl(CPULinxState *env, unsigned dst_tile,
    ...
    const uint32_t impl_op = linx_tile_tepl_impl_selector(canonical_op);
    const uint64_t bytes64 =
        size_code < 60u ? (1ull << (size_code + 4u)) : 0ull;
    ...
    const bool is_row_reduce = linx_tile_tepl_impl_is_row_reduce(impl_op);
    const bool is_col_reduce = linx_tile_tepl_impl_is_col_reduce(impl_op);
    const bool reduce_dst_legal =
        is_col_reduce
            ? linx_tile_tepl_col_reduce_dst_legal(env, impl_op,
                                                  (uint32_t)bytes64,
                                                  elem_bytes)
            : (is_row_reduce
                   ? linx_tile_tepl_row_reduce_dst_legal(env, impl_op,
                                                         (uint32_t)bytes64,
                                                         elem_bytes)
                   : false);
    ...
    if (!linx_tile_tepl_selector_accepted(canonical_op) ||
        ...
        impl_op == UINT32_MAX ||
        !linx_tile_tepl_impl_dtype_supported(impl_op, operation_dtype) ||
        (impl_op == 0x00du &&
         (!has_src0 ||
          !linx_tile_tepl_impl_dtype_supported(impl_op, src0_dtype))) ||
        source_count != (unsigned)linx_tile_tepl_impl_source_arity(impl_op) ||
        dst_tile >= LINX_TILE_SLOT_COUNT || bytes64 == 0u ||
        bytes64 > LINX_TILE_MAX_BYTES ||
        (elem_bytes != 1u && elem_bytes != 2u && elem_bytes != 4u &&
         elem_bytes != 8u) ||
        ((is_col_reduce || is_row_reduce)
             ? !reduce_dst_legal
    ...
    if (is_row_reduce &&
        !linx_tile_tepl_row_reduce_shape(
            env, elem_bytes, (uint32_t)(bytes64 / elem_bytes),
            &rows, &cols, &physical_cols)) {
        return false;
    }
    ...
    if (elem_bytes == 8u && linx_tile_tepl64_core_op(op)) {
        ...
        } else if (is_row_reduce) {
            unsigned dst_stride = 1u;
            if (!linx_tile_tepl_row_reduce_dst_stride(
                    env, (uint32_t)bytes64, 8u, &dst_stride) ||
                !linx_tile_set_shape(env, dst_tile, 1u, rows, dst_stride,
                                     (uint32_t)(bytes64 /
                                                (dst_stride * 8u)))) {
                return false;
            }
    ...
    if (op == 0x02bu) {
        if (!has_src0 || !has_src1 ||
            !linx_tile_tepl_tcmp(env, dst_tile, src0_tile, src1_tile, rows,
        ...
        for (uint32_t r = 0; r < rows; r++) {
            for (uint32_t c = 0; c < cols; c++) {
                ...
                if (op == 0x101u) {
                    ...
                    const uint32_t product = linx_tile_tepl_binary_word(
                        env, 0x002u, dtype, value, scalar);
                    result = linx_tile_tepl_binary_word(env, 0x000u, dtype,
                                                        destination, product);
                } else if (linx_tile_dtype_is_signed(dtype) &&
                           linx_tile_sign_extend(value, elem_bytes) < 0) {
                    result = linx_tile_tepl_binary_word(env, 0x002u, dtype,
                                                        value, scalar);
        ...
        /*
         * TINSERT.md: LB0/LB1 are the source sub-tile ValidCol/ValidRow,
         * LB2 (Col) is the destination tile total column count / row stride.
         * The destination window bounds are therefore checked against LB2 and
         * the destination row count derived from the allocation size, not
         * against the source window shape carried in LB0/LB1.
         */
        const uint64_t dst_cols = physical_cols;
        const uint64_t dst_elem_count = bytes64 / elem_bytes;
        if (dst_cols == 0u || elem_bytes == 0u ||
            dst_elem_count % dst_cols != 0u ||
            row_off + src_rows > dst_elem_count / dst_cols ||
            col_off + src_cols > dst_cols) {
            return false;
        }
        ...
        for (uint32_t out = 0; out < active; out++) {
            ...
            if (li < ln && ri < rn) {
                const uint32_t minv = linx_tile_tepl_binary_word(
        ...
        if (!linx_tile_tepl_product(env, dst_tile, src0_tile, op == 0x035u,
                                    rows, cols, physical_cols,
                                    (uint32_t)bytes64)) {
            return false;
        }
        ...
        if (!linx_tile_tepl_arg_reduce(env, dst_tile, src0_tile, row_reduce,
        ...
        if (!linx_tile_tepl_partial_binary(env, dst_tile, src0_tile, src1_tile,
        ...
        if (!linx_tile_tepl_concat(env, dst_tile, src0_tile, src1_tile, rows,
                                   cols, physical_cols)) {
            return false;
        }
    } else if (op == 0x089u) {
        if (!linx_tile_tepl_gatherb(env, dst_tile, src0_tile, src1_tile, rows,
                                    cols, physical_cols)) {
            return false;
        }
    } else if (op == 0x085u) {
        if (!linx_tile_tepl_extract(env, dst_tile, src0_tile, rows, cols,
                                    physical_cols)) {
            return false;
        }
    } else if (op == 0x084u) {
        if (!linx_tile_tepl_dequant(env, dst_tile, sources[0], sources[1],
        ...
        for (uint32_t i = 0; i < count; i++) {
            ...
            if (!linx_tile_get_elem64(env, src0_tile, lane, src0_elem_bytes,
                ...
                !linx_tile_set_elem64(
                    env, dst_tile, lane, elem_bytes,
                    linx_tile_tepl_convert64(env, value, src0_dtype,
        ...
        unsigned dst_stride = 1u;
        if (!linx_tile_tepl_row_reduce_dst_stride(
                env, (uint32_t)bytes64, elem_bytes, &dst_stride)) {
            return false;
        }
        for (uint32_t r = 0; r < rows; r++) {
            uint32_t result = 0;
        ...
        /*
         * Row-reduce destination is a column vector, one value per row
         * (TROWMAX.md: "dst column/vector tile, one value per row").  The
         * destination allocation holds rows x stride elements: a dense
         * 1-column vector when the TSize has one element per row, or a
         * row-strided tile when the compiler allocated
         * rows x LB2 x elem (e.g. TileM x 8 with ValidCol=1 in
         * normalize_weight_pto.hpp).  Publish the stride so consumers
         * (TROWEXPAND, TADDS, TSTORE, ...) read each row at its own lane.
         */
        env->tile_reg_bytes[dst_tile] = (uint32_t)bytes64;
        linx_tile_set_elem_bytes(env, dst_tile, elem_bytes);
        linx_tile_set_dtype(env, dst_tile, operation_dtype);
        if (!linx_tile_set_shape(env, dst_tile, 1u, rows, dst_stride,
                                 (uint32_t)(bytes64 /
                                            (dst_stride * elem_bytes)))) {
            return false;
        }
        return true;
        ...
        if (!linx_tile_tepl_expand(env, dst_tile, src0_tile, src1_tile,
        ...
        for (uint32_t i = 0; i < active; i++) {
            ...
            if (has_src0 && has_src1) {
                ...
                result =
                    linx_tile_tepl_unary_word(env, op, operation_dtype, lhs);
            } else {
                return false;
            }
            linx_tile_set_elem(env, dst_tile, lane, elem_bytes, result);
    ...
    linx_tile_set_elem_bytes(env, dst_tile, elem_bytes);
    linx_tile_set_dtype(env, dst_tile, operation_dtype);
    if (!linx_tile_set_block_shape(env, dst_tile, (uint32_t)bytes64,
                                   elem_bytes)) {
        return false;
    }
    return true;
                           ...
                           unsigned size_code, uint64_t stride_bytes)
{
    ...
    const LinxTileFormatDesc fmt =
        linx_tile_effective_transfer_format(env, LINX_TMA_GM_TO_TR);
    if (!fmt.valid) {
        helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
        return;
    }
                            ...
                            unsigned size_code, uint64_t stride_bytes)
{
    ...
    const uint32_t tile_elems = (uint32_t)(bytes / elem_bytes);
    const LinxTileFormatDesc fmt =
        linx_tile_effective_transfer_format(env, LINX_TMA_TR_TO_GM);
    if (!fmt.valid) {
        helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
        return;
    }
...
static bool linx_tile_cube_dst_legal(const CPULinxState *env,
                                         uint32_t bytes)
{
    const unsigned m = linx_tile_cube_dim(env->lb[0]);
    const unsigned n = linx_tile_cube_dim(env->lb[1]);
    const uint8_t acc_dtype = linx_tile_numeric_acc_dtype(env->tile_dtype);
    const unsigned acc_bytes = acc_dtype == UINT8_MAX
        ? 0u : linx_tile_dtype_elem_bytes(acc_dtype);

    /* CUBE D is an AccType (S32/F32) M x N Local Tile; LB0=M, LB1=N, LB2=K. */
    return acc_bytes != 0u && m != 0u && n != 0u &&
           bytes >= (uint64_t)m * n * acc_bytes &&
           bytes % acc_bytes == 0u &&
...
{
    return linx_tile_cube_compute_057(env, src_a, src_b, row_scale,
...
{
    /*
     * B.IOR.md (pto-spec@a7f9aabe): the authored list is
     * `B.IOR RegSrc0, RegSrc1, RegSrc2, ->RegDst` with RegSrc0 at
     * [19:15] and RegSrc1 at [24:20].  Keep both RI consumers on this
     * canonical order.
     */
                                            ...
                                            unsigned *size_code_out)
{
    ...
    const unsigned reg_dst = desc & 0x1fu;
    const unsigned size_class = reg_dst >> 2;

    if ((reg_dst & 0x3u) != 0u || size_class == 0u || size_class > 7u) {
        return false;
    }
    /* SharedTSize is PE-local: class 1 is 128 B. */
...
static bool linx_tile_tile_is_bound_source_at(const CPULinxState *env,
                                                  unsigned index,
                                                  unsigned tile)
{
    if (index >= LINX_TILE_MAX_IOT) {
        return false;
    }
    for (unsigned source = 0; source < 2; source++) {
        if ((env->tile_iot_src_valid[index] & (1u << source)) != 0u &&
            env->tile_iot_src_phys[index][source] == tile) {
            return true;
        }
    }
    return false;
                                            ...
                                            unsigned index)
{
    ...
    if (count_by_hand[hand] == LINX_TILE_HAND_DEPTH) {
        ...
        /* The reservation selected the entry that ages out at this commit. */
        if (oldest / LINX_TILE_HAND_DEPTH != hand || oldest != tile ||
            (env->tile_pin_owner[oldest] != 0u &&
             !linx_tile_tile_is_bound_source_at(env, index, oldest))) {
            return false;
        }
        live[hand] &= ~LINX_TILE_HAND_BIT(oldest_depth);
        order[hand][LINX_TILE_HAND_DEPTH - 1u] = 0u;
    ...
    unsigned *size_code_out)
{
    ...
    if (env->tile_iot_count == 0u ||
        ...
        linx_tile_tepl_source_arity(op) < 0 ||
        *source_count_out != (unsigned)linx_tile_tepl_source_arity(op)) {
        return false;
    }
    ...
    for (uint32_t group = 0; group < extent; group += 32u) {
        ...
        for (unsigned pass = 0; pass < 32u; pass++) {
            for (uint32_t i = group; i + 1u < end; i++) {
                ...
                const uint32_t preferred = linx_tile_tepl_binary_word(
...
static bool linx_tile_collect_matrix_bindings(
    const CPULinxState *env, unsigned required_sources,
    unsigned sources[LINX_TILE_MAX_IOT * 2], unsigned *dst_tile_out,
    unsigned *output_index_out, unsigned *size_code_out)
{
    unsigned source_count = 0;
    unsigned output_count = 0;

    if (env->tile_iot_count == 0u ||
        !env->tile_fixp_attr_valid || env->tile_fixp_attr != 0u ||
        !linx_tile_collect_sources(env, sources, &source_count) ||
    ...
    for (unsigned i = 0; i < env->tile_iot_count; i++) {
        const LinxTileIOTDesc desc = linx_tile_decode_iot(env->tile_iot_desc[i]);
        const bool final_desc = i + 1u == env->tile_iot_count;

        if ((desc.last != 0u) != final_desc) {
            return false;
        }
        if (!env->tile_iot_output_valid[i]) {
            if (desc.has_size) {
        ...
        if (++output_count != 1u || !desc.has_size ||
            !linx_tile_size_code_valid(desc.size & 0x1fu) ||
            !linx_tile_get_bound_output(env, i, dst_tile_out)) {
            return false;
        }
        *output_index_out = i;
                                         ...
                                         LinxTMATransferDir dir)
{
    ...
    const LinxTileLayout tr_layout = dir == LINX_TMA_GM_TO_TR
                                      ? fmt.dst : fmt.src;
    return linx_tile_layout_shape_valid(tr_layout, tr_outer, tr_inner,
                                        elem_bytes);
}

static bool linx_tile_preflight_tma(
    ...
    uint8_t *planned_carrier_valid, uint8_t *planned_carrier)
{
    ...
    if (func == LINX_TMA_TLOAD && env->tile_shared_binder_count != 0u) {
        ...
        const uint64_t bytes = UINT64_C(1) << (size_code + 4u);
        const bool legacy_int8_profile =
            dtype == 19u && size_code == 5u &&
            env->lb[0] == 32u && env->lb[1] == 16u &&
            env->lb[2] == 32u;
        const bool fp32_profile =
            dtype == 1u && env->lb[0] != 0u && env->lb[1] != 0u &&
            env->lb[2] != 0u && env->lb[0] <= env->lb[2] &&
            bytes <= LINX_SHARED_TILE_MAX_BYTES &&
            bytes == (uint64_t)env->lb[1] * env->lb[2] * sizeof(float);

        return (legacy_int8_profile || fp32_profile) &&
    ...
    case LINX_TMA_MSCATTER_MASK: {
        ...
        const unsigned required_sources =
            (env->tile_func & 0x1f) == LINX_TMA_MGATHER ? 1u :
            (env->tile_func & 0x1f) == LINX_TMA_MGATHER_MASK ? 2u :
    ...
    for (unsigned i = 0; i < count; i++) {
        ...
        case LINX_TMA_TMOV: {
            ...
            if (env->tile_reg_bytes[src_tile] < (1ull << (size_code + 4u))) {
                return false;
            }
            linx_tile_consume_bound_sources(env, planned_live, i, &d,
                                            NULL, NULL,
...
void HELPER(linx_tile_append_shared_binder)(CPULinxState *env, uint32_t shared)
{
    ...
    for (unsigned i = 0; i < env->tile_shared_binder_count; i++) {
        if (env->tile_shared_binder[i] == (shared & 0xffu)) {
            helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
            return;
        }
    }
...
void HELPER(linx_tile_append_iot)(CPULinxState *env, uint64_t packed)
{
    ...
    const uint32_t tma_func = env->tile_func & 0x1f;
    ...
    if (env->tile_shared_binder_count != 0u &&
        ((!shared_tmov && !shared_cube) ||
         (shared_tmov && env->tile_shared_binder_count != 1u) ||
         (shared_cube &&
          ((env->tile_func & 0x1fu) >= LINX_CUBE_TGEMV ||
           (env->tile_shared_binder_count != 1u &&
    ...
    if (env->blocktype == LINX_BLOCK_TEPL &&
        (!linx_tile_tepl_selector_accepted(env->tile_func & 0x7fu) ||
         !linx_tile_tepl_dtype_supported(env->tile_func & 0x7fu,
    ...
    if (desc.has_size && (vector_block || tile_output)) {
        uint16_t staged_live[LINX_TILE_HAND_COUNT];
        uint16_t staged_reserved[LINX_TILE_HAND_COUNT];
        uint8_t staged_order[LINX_TILE_HAND_COUNT][LINX_TILE_HAND_DEPTH];
        uint8_t staged_count[LINX_TILE_HAND_COUNT];
        unsigned dst_tile;
        const uint64_t bytes64 = desc.has_size && desc.size < 60u
                                 ? (1ull << (desc.size + 4u)) : 0;

        memcpy(staged_live, env->tile_hand_live, sizeof(staged_live));
        memcpy(staged_reserved, env->tile_hand_reserved,
               sizeof(staged_reserved));
        memcpy(staged_order, env->tile_hand_order, sizeof(staged_order));
        memcpy(staged_count, env->tile_hand_count, sizeof(staged_count));
        for (unsigned planned = 0; planned < index; planned++) {
            if (env->tile_iot_output_valid[planned]) {
                ...
                const unsigned hand = tile / LINX_TILE_HAND_DEPTH;
                const unsigned depth = tile % LINX_TILE_HAND_DEPTH;
                staged_reserved[hand] |= LINX_TILE_HAND_BIT(depth);
                if (!linx_tile_stage_output(staged_live, staged_reserved,
                                             staged_order, staged_count,
                                             tile)) {
                    helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
                    return;
                }
        ...
        if (bytes64 == 0 || bytes64 > LINX_TILE_MAX_BYTES ||
            (bytes64 & 3u) != 0 ||
            !linx_tile_reserve_output(
                env, staged_live, staged_reserved, staged_order, staged_count,
                &desc, index, &dst_tile)) {
        ...
        const uint32_t tepl_op = env->tile_func & 0x7fu;
        ...
        uint32_t group_left_bytes = 0u;
        uint32_t group_right_bytes = 0u;
        uint32_t group_output_bytes = 0u;
        unsigned group_output_size_code = 0u;
        const bool cooperative_matrix_output =
            env->blocktype == LINX_BLOCK_CUBE &&
            env->tile_shared_binder_count != 0u &&
            linx_tile_group_fp32_sizes(env, &group_left_bytes,
                                       &group_right_bytes,
                                       &group_output_bytes,
                                       &group_output_size_code) &&
            bytes64 == group_output_bytes;
        /*
         * Local TMOV is byte-preserving (TMOV.md: dtypes "byte-preserving;
         * descriptors must be compatible"); its header DataType is not
         * consumed and the destination size class is the only sizing
         * constraint, so skip the dtype-derived shape gate.
         */
        const bool tmov_byte_preserving =
            env->blocktype == LINX_BLOCK_TMA &&
            (env->tile_func & 0x1fu) == LINX_TMA_TMOV;
        const bool cube_dst_legal =
            env->blocktype == LINX_BLOCK_CUBE &&
            linx_tile_cube_dst_legal(env, (uint32_t)bytes64);
        const bool tepl_reduce_dst_legal =
            env->blocktype == LINX_BLOCK_TEPL &&
            (linx_tile_tepl_col_reduce_dst_legal(
                 ...
                 linx_tile_dtype_elem_bytes(
                     linx_tile_effective_dtype(env))) ||
             linx_tile_tepl_row_reduce_dst_legal(
                 env,
                 linx_tile_tepl_impl_selector(env->tile_func & 0x7fu),
                 (uint32_t)bytes64,
                 linx_tile_dtype_elem_bytes(
        ...
        if (!cooperative_matrix_output &&
            !tmov_byte_preserving &&
            !tepl_reduce_dst_legal && !cube_dst_legal &&
    ...
    /* Freeze source provenance and pin each bound reader until commit. */
    for (unsigned source = 0; source < 2; source++) {
        if ((src_valid & (1u << source)) != 0u) {
            env->tile_pin_owner[src_phys[source]] |= owner;
...
static bool linx_tile_preflight_cube(const CPULinxState *env)
{
    ...
    unsigned a_index = 0;
    unsigned b_index = 1;
    unsigned scale_a_index = 0;
    unsigned scale_b_index = 0;
    unsigned bias_index = 0;
    ...
    if (!linx_tile_collect_matrix_bindings(
            env, required, sources, &dst_tile, &output_index, &size_code) ||
        !linx_tile_cube_primary_legal(env, sources[a_index], sources[b_index],
                                      false)) {
        return false;
    }
    if (with_acc) {
        const unsigned m = linx_tile_cube_dim(env->lb[0]);
        const unsigned n = linx_tile_cube_dim(env->lb[1]);
        const uint8_t acc_dtype = with_scale ? LINX_TILE_ACC_FP32 :
            linx_tile_numeric_acc_dtype(env->tile_dtype);
        if (!linx_tile_cube_operand_legal(env, sources[0], acc_dtype, m, n)) {
            return false;
        }
    }
    if (with_scale &&
        !linx_tile_cube_scale_legal(env, sources[scale_a_index],
                                    sources[scale_b_index])) {
        return false;
    }
    if (with_bias && !linx_tile_cube_bias_legal_for(
            env, sources[bias_index], with_scale ? 1u : env->tile_dtype)) {
        return false;
    }
    /* The destination is only materialized after this preflight succeeds. */
    const uint64_t allocated = UINT64_C(1) << (size_code + 4u);
    const uint64_t required_bytes =
        (uint64_t)linx_tile_cube_dim(env->lb[0]) *
        linx_tile_cube_dim(env->lb[1]) * sizeof(uint32_t);
    return allocated >= required_bytes;
}
                                         ...
                                         unsigned *dst_d_out)
{
    ...
    unsigned size_code = 0;
    unsigned source_count = 0;
    uint32_t left_bytes = 0u;
    uint32_t right_bytes = 0u;
    uint32_t output_bytes = 0u;
    unsigned output_size_code = 0u;
    bool valid;
    ...
    *dst_d_out = UINT_MAX;
    if (cpu->core4 == NULL ||
        ...
        !linx_tile_datr_applicable(
            env->blocktype, env->tile_func, env->tile_attr_raw) ||
        env->tile_dtype != 1u || env->tile_shared_binder_count != 1u ||
        !linx_tile_group_fp32_sizes(env, &left_bytes, &right_bytes,
                                    &output_bytes, &output_size_code) ||
        !linx_tile_collect_sources(env, sources, &source_count) ||
        source_count != (func == LINX_CUBE_TMATMUL_ACC ? 2u : 1u) ||
        !linx_tile_collect_matrix_bindings(
            env, source_count, sources, dst_d_out, &output_index,
            &size_code) || size_code != output_size_code) {
        return false;
    }
    for (unsigned i = 0; i < env->tile_iot_count; i++) {
        const LinxTileIOTDesc desc = linx_tile_decode_iot(env->tile_iot_desc[i]);
    ...
    if (func == LINX_CUBE_TMATMUL_ACC) {
        *src_c_out = sources[0];
        *src_a_out = sources[1];
        if (!linx_tile_cube_operand_legal(
                env, *src_c_out, LINX_TILE_ACC_FP32,
                env->lb[0], env->lb[1])) {
            return false;
        }
    } else {
    ...
    if (!linx_tile_cube_operand_legal(env, *src_a_out, 1u,
                                      env->lb[0], env->lb[2]) ||
        env->tile_reg_bytes[*src_a_out] != left_bytes ||
        (func == LINX_CUBE_TMATMUL_ACC &&
         env->tile_reg_bytes[*src_c_out] != output_bytes)) {
        return false;
    }
    ...
    valid = shared->ready && shared->defined_mask == 0xfu &&
            shared->dtype == 1u &&
            shared->bytes == right_bytes * LINX_CORE4_PE_COUNT;
    qemu_mutex_unlock(&cpu->core4->lock);
    return valid;
}
...
static inline uint32_t linx_tile_tmov_source_dtype(const CPULinxState *env,
                                                   unsigned iot_index)
{
    const uint8_t valid = env->tile_iot_src_valid[iot_index];
    const unsigned src = (valid & 1u) != 0u
                             ? env->tile_iot_src_phys[iot_index][0]
                             : ((valid & 2u) != 0u
                                    ? env->tile_iot_src_phys[iot_index][1]
                                    : LINX_TILE_SLOT_COUNT);
    if (src < LINX_TILE_SLOT_COUNT &&
        linx_tile_dtype_elem_bytes(env->tile_reg_dtype[src]) != 0u) {
        return env->tile_reg_dtype[src] & 0x1fu;
    }
    return env->tile_dtype & 0x1fu;
}
...
/*
 * Execute TEPL once against snapshotted destination state before the
 ...
 */
static bool linx_tile_preflight_tepl(CPULinxState *env)
{
    const uint32_t op = env->tile_func & 0x7fu;
    const uint32_t impl = linx_tile_tepl_impl_selector(op);
    unsigned sources[LINX_TILE_MAX_IOT * 2];
    unsigned source_count = 0;
    unsigned output_index = 0;
    unsigned dst_tile = 0;
    ...
    if (!linx_tile_tepl_selector_accepted(op) ||
        !linx_tile_tepl_selector_executable(op) || impl == UINT32_MAX ||
        !linx_tile_tepl_dtype_supported(op, linx_tile_effective_dtype(env))) {
        return false;
    }
    if (impl == 0x106u) {
    ...
    if (legal && impl == 0x106u) {
        ...
        legal = linx_tile_tepl(env, dst_tile, sources, source_count,
                               size_code, op);
    }
    ...
    uint8_t *acc_sources_valid, uint8_t acc_src0, uint8_t acc_src1)
{
    for (unsigned i = 0; i < env->tile_iot_count; i++) {
        ...
        const uint32_t matrix_func = env->tile_func & 0x1fu;
        const bool matrix_mx = matrix_func == LINX_CUBE_TMATMUL_MX ||
                               matrix_func == LINX_CUBE_TMATMUL_MX_BIAS ||
                               matrix_func == LINX_CUBE_TMATMUL_MX_ACC ||
                               matrix_func == LINX_CUBE_TGEMV_MX ||
                               matrix_func == LINX_CUBE_TGEMV_MX_BIAS ||
                               matrix_func == LINX_CUBE_TGEMV_MX_ACC;
        const bool tmov = env->blocktype == LINX_BLOCK_TMA &&
                          (env->tile_func & 0x1fu) == LINX_TMA_TMOV;
        /*
         * TMOV is byte-preserving (TMOV.md: "byte-preserving; descriptors
         * must be compatible") and does not consume the header DataType;
         * the destination inherits the bound source Tile dtype so the
         * materialized metadata stays consistent with the copied payload.
         */
        const uint32_t output_dtype = env->blocktype == LINX_BLOCK_CUBE
            ? (matrix_mx ? LINX_TILE_ACC_FP32 :
               linx_tile_numeric_acc_dtype(env->tile_dtype))
            : (tmov ? linx_tile_tmov_source_dtype(env, i)
                    : linx_tile_effective_dtype(env));
        const unsigned elem_bytes = linx_tile_dtype_elem_bytes(output_dtype);
        const uint32_t tepl_op = env->tile_func & 0x7fu;
        const bool in_place = env->blocktype == LINX_BLOCK_TEPL &&
                              (tepl_op == 0x02fu);
        ...
        /*
         * A full-hand destination reserve may reuse the very slot that this
         * byte-preserving TMOV reads (Q#16 push).  Do not clear that backing
         * storage before the move; the TMOV execution then preserves the
         * payload as a self copy.
         */
        const bool tmov_aliased_source =
            tmov && env->tile_iot_src_phys[i][0] == dst_tile;
        if (!tmov_aliased_source) {
            memset(env->tile_reg[dst_tile], 0, sizeof(env->tile_reg[dst_tile]));
        }
        ...
        if (env->blocktype == LINX_BLOCK_CUBE) {
            ...
            if (cols == 0u || dst_elem == 0u ||
                ...
                return false;
                   ...
                   linx_tile_tepl_impl_is_row_reduce(
                       linx_tile_tepl_impl_selector(
                           env->tile_func & 0x7fu))) {
            /*
             * Row-reduce destination is a column vector of LB1 values
             * (TROWMAX.md: "dst column/vector tile, one value per row"),
             * not the 2D source shape carried in B.DIM.
             */
            const uint32_t impl =
                linx_tile_tepl_impl_selector(env->tile_func & 0x7fu);
            const unsigned dst_elem =
                linx_tile_tepl_impl_is_arg_reduce(impl) ? 4u : elem_bytes;
            const unsigned rows = (unsigned)(env->lb[1] & 0xffffu);
            unsigned dst_stride = 1u;
            if (rows == 0u || dst_elem == 0u ||
                !linx_tile_tepl_row_reduce_dst_stride(env, bytes, dst_elem,
                                                      &dst_stride) ||
                !linx_tile_set_shape(env, dst_tile, 1u, rows, dst_stride,
...
static bool linx_tile_group_mma_commit(CPULinxState *env, uint64_t resume_pc)
{
    ...
    if (!linx_tile_group_cube_profile(env, &src_a, &src_c, &dst_d)) {
        if (core4 != NULL) {
    ...
    if (core4->collective_arrived == 0u) {
        ...
        core4->collective_m = env->lb[0];
        core4->collective_n = env->lb[1];
        core4->collective_k = env->lb[2];
    } else {
    ...
    LinxTileRegSnapshot *dst_snapshots =
        g_new(LinxTileRegSnapshot, LINX_CORE4_PE_COUNT);
    const uint32_t m = core4->collective_m;
    const uint32_t n = core4->collective_n;
    const uint32_t output_bytes = m * n * sizeof(uint32_t);
    unsigned output_size_code = 0u;
    valid = linx_tile_size_code_from_bytes(output_bytes, &output_size_code);
    for (unsigned i = 0; i < LINX_CORE4_PE_COUNT; i++) {
        CPULinxState *peer = &core4->cpu[i]->env;
        linx_tile_snapshot_acc(peer, &acc_snapshots[i]);
        linx_tile_snapshot_reg(peer, core4->collective_dst[i], &dst_snapshots[i]);
        if (with_acc) {
            memcpy(peer->tile_acc, peer->tile_reg[core4->collective_acc[i]],
                   output_bytes);
            peer->tile_acc_bytes = output_bytes;
            peer->tile_acc_dtype = LINX_TILE_ACC_FP32;
            peer->tile_acc_valid = 1u;
            peer->tile_acc_cols = n;
            peer->tile_acc_rows = m;
        }
        const uint32_t right_bytes =
            core4->collective_k * core4->collective_n * sizeof(uint32_t);
        const uint8_t *shared_right = shared->data + i * right_bytes;

        valid = valid && linx_tile_cube_compute_shared_b_057(
            peer, core4->collective_src[i], shared_right,
            right_bytes, shared->dtype, output_size_code, with_acc);
        if (!valid) {
            break;
        }
        const unsigned dst = core4->collective_dst[i];
        memcpy(peer->tile_reg[dst], peer->tile_acc, output_bytes);
        peer->tile_reg_bytes[dst] = output_bytes;
        peer->tile_reg_elem_bytes[dst] = sizeof(uint32_t);
        peer->tile_reg_dtype[dst] = LINX_TILE_ACC_FP32;
        peer->tile_reg_valid_cols[dst] = n;
        peer->tile_reg_valid_rows[dst] = m;
        peer->tile_reg_cols[dst] = n;
        peer->tile_reg_rows[dst] = m;
    }
    if (!valid) {
        for (unsigned i = 0; i < LINX_CORE4_PE_COUNT; i++) {
            linx_tile_restore_acc(&core4->cpu[i]->env, &acc_snapshots[i]);
            linx_tile_restore_reg(&core4->cpu[i]->env, &dst_snapshots[i]);
        }
        g_free(dst_snapshots);
    ...
    for (unsigned i = 0; i < LINX_CORE4_PE_COUNT; i++) {
        ...
        memcpy(reserved, peer->tile_hand_reserved, sizeof(reserved));
        memcpy(order, peer->tile_hand_order, sizeof(order));
        memcpy(count_by_hand, peer->tile_hand_count, sizeof(count_by_hand));
        for (unsigned desc_index = 0; desc_index < peer->tile_iot_count;
             desc_index++) {
            const LinxTileIOTDesc desc =
                linx_tile_decode_iot(peer->tile_iot_desc[desc_index]);
            linx_tile_consume_bound_sources(peer, live, desc_index, &desc,
                                            order, count_by_hand,
                                            &carrier_valid, &carrier);
            if (peer->tile_iot_output_valid[desc_index] &&
                !linx_tile_complete_bound_output(
                    peer, live, reserved, order, count_by_hand, desc_index)) {
                linx_tile_group_fail_locked(core4);
                qemu_mutex_unlock(&core4->lock);
                return false;
            }
        ...
        memset(peer->tile_acc, 0, sizeof(peer->tile_acc));
        peer->tile_acc_bytes = 0u;
        peer->tile_acc_dtype = 0u;
        peer->tile_acc_valid = 0u;
        peer->tile_acc_cols = 0u;
        peer->tile_acc_rows = 0u;
        peer->tile_acc_carrier_valid = 0u;
...
void HELPER(linx_tile_commit)(CPULinxState *env, uint64_t resume_pc)
{
    ...
    if (env->blocktype == LINX_BLOCK_TMA) {
        ...
        txn_gate.operands_legal = linx_tile_preflight_tma(
            env, planned_live, &planned_carrier_valid, &planned_carrier);
    } else if (env->blocktype == LINX_BLOCK_TEPL) {
        bool allocation_fault = false;
        if (linx_tile_tepl_impl_selector(env->tile_func & 0x7fu) == 0x10au &&
        ...
        if (txn_gate.operands_legal) {
            txn_gate.operands_legal = linx_tile_preflight_tepl(env);
        }
    ...
    case LINX_BLOCK_TEPL: {
        ...
        unsigned size_code = 0;
        const uint32_t op = env->tile_func & 0x7fu;
        const uint32_t impl_op = linx_tile_tepl_impl_selector(op);

        if (!linx_tile_tepl_selector_accepted(op) || impl_op == UINT32_MAX) {
            helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
            break;
        }
        ...
        linx_tile_invalidate_acc_sources_on_output(
            dst_tile, &acc_sources_valid, acc_src0, acc_src1);
        if (!linx_tile_tepl(env, dst_tile, sources, source_count,
                            size_code, op)) {
            helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
            break;
        }
    ...
    case LINX_BLOCK_TMA:
        ...
        case LINX_TMA_TLOAD: {
            if (shared_tload) {
                ...
                const size_t local_bytes = UINT64_C(1) << (size_code + 4u);
                const size_t aggregate_bytes =
                    local_bytes * LINX_CORE4_PE_COUNT;
                g_autofree uint8_t *staged = g_malloc(aggregate_bytes);
                const uint64_t base = env->gpr[addr_reg];
                for (size_t i = 0; i < aggregate_bytes; i++) {
                    staged[i] = cpu_ldub_data(env, (abi_ptr)(base + i));
                }
                LinxSharedTileVersion *shared =
                    &cpu->core4->shared_tile[env->tile_shared_binder[0]];
                qemu_mutex_lock(&cpu->core4->lock);
                memcpy(shared->data, staged, aggregate_bytes);
                memset(shared->data + aggregate_bytes, 0,
                       LINX_SHARED_TILE_MAX_BYTES - aggregate_bytes);
                shared->bytes = aggregate_bytes;
                shared->dtype = linx_tile_effective_dtype(env);
                shared->producer_bpc = env->bpc;
                shared->defined_mask = 0xfu;
                shared->ready = 1u;
        ...
        case LINX_TMA_TMOV: {
            ...
            for (unsigned i = 0; i < count; i++) {
                ...
                const uint64_t bytes64 = (size_code < 60u) ? (1ull << (size_code + 4u)) : 0ull;
                if (bytes64 == 0 || bytes64 > LINX_TILE_MAX_BYTES ||
                    src_tile >= LINX_TILE_SLOT_COUNT ||
                    dst_tile >= LINX_TILE_SLOT_COUNT ||
                ...
                linx_tile_invalidate_acc_sources_on_output(
                    dst_tile, &acc_sources_valid, acc_src0, acc_src1);
                memcpy(env->tile_reg[dst_tile], env->tile_reg[src_tile],
                       (size_t)bytes64);
                memset((uint8_t *)env->tile_reg[dst_tile] + bytes64, 0,
                       LINX_TILE_MAX_BYTES - (size_t)bytes64);
                env->tile_reg_bytes[dst_tile] = (uint32_t)bytes64;
                linx_tile_set_elem_bytes(env, dst_tile,
                                         env->tile_reg_elem_bytes[src_tile]);
                linx_tile_set_dtype(env, dst_tile,
                                    env->tile_reg_dtype[src_tile]);
    ...
    case LINX_BLOCK_CUBE:
        ...
        case LINX_CUBE_TGEMV_MX_ACC: {
            ...
            const bool with_bias = func == LINX_CUBE_TMATMUL_BIAS ||
                                   func == LINX_CUBE_TMATMUL_MX_BIAS ||
                                   func == LINX_CUBE_TGEMV_BIAS ||
                                   func == LINX_CUBE_TGEMV_MX_BIAS;
            const bool with_acc = func == LINX_CUBE_TMATMUL_ACC ||
                                  func == LINX_CUBE_TMATMUL_MX_ACC ||
                                  func == LINX_CUBE_TGEMV_ACC ||
                                  func == LINX_CUBE_TGEMV_MX_ACC;
            const unsigned required = mx ? (with_acc || with_bias ? 5u : 4u)
                                         : (with_acc || with_bias ? 3u : 2u);
            const unsigned a_index = with_acc ? 1u : 0u;
            const unsigned b_index = mx ? (with_acc ? 3u : 2u)
                                        : (with_acc ? 2u : 1u);
            const unsigned scale_a_index = with_acc ? 2u : 1u;
            const unsigned scale_b_index = with_acc ? 4u : 3u;
            const unsigned bias_index = mx ? 4u : 2u;
            const unsigned rows = linx_tile_cube_dim(env->lb[0]);
            const unsigned cols = linx_tile_cube_dim(env->lb[1]);
            const uint8_t acc_dtype = mx ? LINX_TILE_ACC_FP32 :
                linx_tile_numeric_acc_dtype(env->tile_dtype);
            const uint32_t result_bytes = rows * cols * sizeof(uint32_t);

            if (!linx_tile_collect_matrix_bindings(
                    env, required, sources, &dst_tile, &output_index,
                    &size_code)) {
                helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
                break;
            }
            if (with_acc) {
                ...
                memcpy(env->tile_acc, env->tile_reg[sources[0]], result_bytes);
                env->tile_acc_bytes = result_bytes;
                env->tile_acc_dtype = acc_dtype;
                env->tile_acc_valid = 1u;
                env->tile_acc_cols = cols;
                env->tile_acc_rows = rows;
            }
            if (!linx_tile_cube_compute(
                    env, sources[a_index], sources[b_index],
                    mx ? sources[scale_a_index] : 0u,
                    mx ? sources[scale_b_index] : 0u,
                    with_bias ? sources[bias_index] : 0u, size_code,
                    mx, with_bias, with_acc) || env->tile_acc_bytes < result_bytes) {
                helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
                break;
            }
            memcpy(env->tile_reg[dst_tile], env->tile_acc, result_bytes);
            env->tile_reg_bytes[dst_tile] = result_bytes;
            env->tile_reg_elem_bytes[dst_tile] = sizeof(uint32_t);
            env->tile_reg_dtype[dst_tile] = acc_dtype;
            env->tile_reg_valid_cols[dst_tile] = cols;
            env->tile_reg_valid_rows[dst_tile] = rows;
            env->tile_reg_cols[dst_tile] = cols;
            env->tile_reg_rows[dst_tile] = rows;
            for (unsigned i = 0; i < env->tile_iot_count; i++) {
                const LinxTileIOTDesc desc =
                    linx_tile_decode_iot(env->tile_iot_desc[i]);
                                         ...
                                         uint64_t resume_pc)
{
    if (env->blocktype == LINX_BLOCK_TMA &&
        (env->tile_func & 0x1fu) == LINX_TMA_TLOAD &&
        env->tile_shared_binder_count == 1u &&
        env->tile_ior_count == 1u && env->tile_iot_count == 0u &&
        env->tile_iot_valid == 1u) {
        HELPER(linx_tile_commit)(env, resume_pc);
    }
}
...
static void linx_tile_commit_vector_bindings(CPULinxState *env)
{
    ...
    /* Consume the header-frozen physical inputs before publishing outputs. */
    for (unsigned i = 0; i < env->tile_iot_count; i++) {
        const LinxTileIOTDesc d = linx_tile_decode_iot(env->tile_iot_desc[i]);

        if ((env->tile_iot_src_valid[i] & 1u) != 0 &&
            (d.flags & LINX_IOT_S0R) == 0) {
            linx_tile_release_source(live, order, count_by_hand,
                                     env->tile_iot_src_phys[i][0], false,
                                     &env->tile_acc_carrier_valid,
                                     &env->tile_acc_carrier);
        }
        if ((env->tile_iot_src_valid[i] & 2u) != 0 &&
            (d.flags & LINX_IOT_S1R) == 0) {
            linx_tile_release_source(live, order, count_by_hand,
                                     env->tile_iot_src_phys[i][1], false,
                                     &env->tile_acc_carrier_valid,
                                     &env->tile_acc_carrier);
        }
    }
...
static bool linx_is_bstart_at_addr(CPULinxState *env, uint64_t pc)
{
    ...
    if (len == 4) {
        ...
        /*
         * PTO defines B.HINT TRACE.{begin,end} as a special empty block
         * marker.  Bit 15 selects begin versus end and is intentionally
         * excluded from the opcode match, so both encodings are valid
