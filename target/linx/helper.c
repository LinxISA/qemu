...
void HELPER(linx_ssr_write)(CPULinxState *env, uint32_t ssrid, uint64_t value)
{
    ...
    default:
        ...
        if (is_manager) {
            ...
            if (idx == LINX_SSR_ECONFIG) {
                value = linx_econfig_sanitize(value);
            }
...
void HELPER(linx_extension_first_use)(CPULinxState *env, uint32_t kind,
                                      uint64_t pc)
{
    if (linx_first_use_prepare(env, (LinxFirstUseKind)kind, pc)) {
        helper_raise_exception(env, LINX_EXCP_EXTENSION_FIRST_USE);
    }
}
...
{
    return size_code >= 3u && size_code <= 12u;
}
...
static bool linx_tile_set_tcvt_shape(CPULinxState *env,
                                     unsigned destination,
                                     unsigned source, uint32_t bytes,
                                     unsigned elem_bytes, bool publish)
{
    ...
    if (!linx_tile_tcvt_descriptor(
            env->tile_reg_valid_cols[source],
            env->tile_reg_valid_rows[source], env->tile_reg_cols[source],
            env->tile_reg_rows[source], bytes, elem_bytes,
            env->lb[0] & 0xffffu, env->lb[1] & 0xffffu,
            env->lb[2] & 0xffffu, &valid_cols, &valid_rows, &cols, &rows)) {
        return false;
    }
    return !publish || linx_tile_set_shape(
        env, destination, valid_cols, valid_rows, cols, rows);
                                 ...
                                 unsigned src)
{
    ...
    env->tile_reg_cube_k_repeat[dst] = env->tile_reg_cube_k_repeat[src];
    env->tile_reg_cube_n_repeat[dst] = env->tile_reg_cube_n_repeat[src];
    env->tile_reg_cube_cell_count[dst] = env->tile_reg_cube_cell_count[src];
    env->tile_reg_cube_storage_bytes[dst] =
        env->tile_reg_cube_storage_bytes[src];
}
...
static inline int64_t linx_tile_sign_extend64(uint64_t value,
                                              unsigned elem_bytes)
{
    if (elem_bytes == 1u) {
        return (int8_t)value;
                                           ...
                                           unsigned elem_bytes)
{
    ...
    if (type >= 6u && type <= 14u) {
        return (float)linx_tile_numeric_decode(dtype, value, 0);
    }
...
static inline uint32_t linx_tile_f32_as_e8m0(float value, uint32_t rmode,
                                          bool sat)
{
    const uint32_t bits = linx_tile_f32_as_word(value);
    ...
    if (exp == 0xffu) {
        if (frac != 0u || sign != 0u) {
            return 0xffu; /* NaN or negative infinity: NV */
        }
        return sat ? 0xfeu : 0xffu; /* positive infinity: NX|OF */
    }
    if (exp == 0u && frac == 0u) {
        return 0xffu; /* positive or negative zero: NV */
    }
    if (sign != 0u) {
        return 0xffu; /* negative finite: NV */
    }

    /*
     * Positive finite value.  frexp gives value = fr * 2^exp2 with
     * 0.5 <= fr < 1, so the floor base-two exponent is exp2 - 1.
     * This mirrors ReferenceFloatToE8M0 in pto-spec
     * asl/arch/profile/e8m0-conversion.asl.
     */
    const double magnitude = (double)value;
    int exp2 = 0;
    const double fr = frexp(magnitude, &exp2);
    const int floor_exponent = exp2 - 1;
    const bool exact_power = fr == 0.5;
    const double fraction = 2.0 * fr - 1.0;

    if (floor_exponent < -127) {
        return sat ? 0x00u : 0xffu;
    }
    if (floor_exponent == 127 && !exact_power) {
        return sat ? 0xfeu : 0xffu;
    }

    int rounded = floor_exponent;
    if (!exact_power) {
        const int ceiling = floor_exponent + 1;
        bool round_up = false;
        switch (rmode & 0x7u) {
        case 2u: /* RTZ */
            round_up = floor_exponent < 0;
            break;
        case 3u: /* RDN / RTM */
            round_up = false;
            break;
        case 4u: /* RUP / RTP */
            ...
            break;
        case 5u: /* RNA */
            round_up = fraction >= 0.5;
            break;
        case 6u: /* RTO */
            round_up = (floor_exponent & 1) == 0;
            break;
        case 7u: /* RHB */
        ...
        default: /* NONE / RNE */
            round_up = fraction > 0.5 ||
                       (fraction == 0.5 && (floor_exponent & 1));
            break;
        }
        rounded = round_up ? ceiling : floor_exponent;
    }
                                              ...
                                              unsigned elem_bytes)
{
    ...
    if (type >= 6u && type <= 14u) {
        if (type == 13u) {
            return linx_tile_f32_as_e8m0(value, rmode, sat);
        }
        if (type == 11u || type == 12u || type == 14u) {
            return (uint32_t)linx_tile_numeric_encode_nibble(
                type, (double)value, rmode, sat);
        }
        return (uint32_t)linx_tile_numeric_encode(
            type, (double)value, rmode, sat);
    }
                                                  ...
                                                  uint32_t lhs, uint32_t rhs)
{
    ...
    if ((dtype & 0x1fu) == 4u) {
        ...
        case 0x030u:
        case 0x032u: {
            const float fa = linx_tile_word_as_f32(
                float32_val(float16_to_float32(a, true, &env->fp_status)));
            const float fb = linx_tile_word_as_f32(
                float32_val(float16_to_float32(b, true, &env->fp_status)));
            const float rem =
                fb == 0.0f ? 0.0f : fa - floorf(fa / fb) * fb;
            out = float32_to_float16(
    ...
    if ((dtype & 0x1fu) == 5u) {
        ...
        case 0x032u: {
            const float fa = linx_tile_word_as_f32(
                float32_val(bfloat16_to_float32(a, &env->fp_status)));
            const float fb = linx_tile_word_as_f32(
                float32_val(bfloat16_to_float32(b, &env->fp_status)));
            const float rem =
                fb == 0.0f ? 0.0f : fa - floorf(fa / fb) * fb;
            out = float32_to_bfloat16(
                make_float32(linx_tile_f32_as_word(rem)), &env->fp_status);
            break;
        }
        default:
            return 0u;
...
static bool linx_tile_operation_dequant(CPULinxState *env, unsigned dst_tile,
                                   ...
                                   uint32_t physical_cols)
{
    /* BSTART.SFU carries the integer source type; B.DATR carries the FP32
     * destination type.  tile_attr_dtype is the latter (bit 0x100 denotes a
     * present B.DATR), so read the source type from tile_dtype. */
    const uint32_t src_dtype = env->tile_dtype & 0x1fu;
    const unsigned src_bytes = linx_tile_dtype_elem_bytes(src_dtype);
    const uint32_t src_stride = env->tile_reg_cols[sources[0]];

    if (source_count == 0u || source_count > 3u ||
        (env->tile_attr_dtype & 0x1fu) != 1u ||
        (src_dtype != 19u && src_dtype != 27u &&
         src_dtype != 18u && src_dtype != 26u) ||
        (env->tile_reg_dtype[sources[0]] & 0x1fu) != src_dtype ||
        env->tile_reg_elem_bytes[sources[0]] != src_bytes ||
        env->tile_reg_valid_rows[sources[0]] != rows ||
        env->tile_reg_valid_cols[sources[0]] != cols ||
        src_stride < cols) {
        return false;
    }

    if (source_count >= 3u) {
        /* Legacy per-row scale/offset tile form (tepl_smoke v0.2). */
        const uint32_t scale_stride = env->tile_reg_cols[sources[1]];
        const uint32_t offset_stride = env->tile_reg_cols[sources[2]];
        if ((env->tile_reg_dtype[sources[1]] & 0x1fu) != 1u ||
            (env->tile_reg_dtype[sources[2]] & 0x1fu) != 1u ||
            env->tile_reg_elem_bytes[sources[1]] != sizeof(uint32_t) ||
            env->tile_reg_elem_bytes[sources[2]] != sizeof(uint32_t) ||
            env->tile_reg_valid_rows[sources[1]] < rows ||
            env->tile_reg_valid_rows[sources[2]] < rows ||
            env->tile_reg_valid_cols[sources[1]] < 1u ||
            env->tile_reg_valid_cols[sources[2]] < 1u) {
            return false;
        }
        for (uint32_t r = 0; r < rows; r++) {
            uint32_t scale_word = 0;
            uint32_t offset_word = 0;
            if (!linx_tile_get_elem(env, sources[1], r * scale_stride,
                                    sizeof(uint32_t), &scale_word) ||
                !linx_tile_get_elem(env, sources[2], r * offset_stride,
            ...
            for (uint32_t c = 0; c < cols; c++) {
                uint32_t src = 0;
                if (!linx_tile_get_elem(env, sources[0], r * src_stride + c,
                                        src_bytes, &src)) {
                    return false;
                }
                const float value =
    ...
    /* Latest PTO v0.58 scalar B.IOR multiplier and source-typed zero point. */
    uint32_t scale_bits = 0x3f800000u;
    uint32_t zero_bits = 0u;
    unsigned scale_reg = 0;
    unsigned zero_reg = 0;
    if (linx_tile_resolve_ior(env, 0, &scale_reg)) {
        scale_bits = (uint32_t)env->gpr[scale_reg];
    }
    if (linx_tile_resolve_ior(env, 1, &zero_reg)) {
        zero_bits = (uint32_t)env->gpr[zero_reg];
    }
    const float scale = linx_tile_word_as_f32(scale_bits);
    if (!(scale > 0.0f) || !isfinite(scale)) {
        return false;
    }
    const uint32_t unsigned_mask = (1u << (src_bytes * 8u)) - 1u;
    const int64_t zero_point = linx_tile_dtype_is_signed(src_dtype)
                                   ? (int64_t)linx_tile_sign_extend(zero_bits, src_bytes)
                                   : (int64_t)(zero_bits & unsigned_mask);
    for (uint32_t r = 0; r < rows; r++) {
        for (uint32_t c = 0; c < cols; c++) {
            uint32_t src = 0;
            if (!linx_tile_get_elem(env, sources[0], r * src_stride + c,
            ...
            const int64_t quantized = linx_tile_dtype_is_signed(src_dtype)
                                          ? (int64_t)linx_tile_sign_extend(src, src_bytes)
                                          : (int64_t)(src & unsigned_mask);
            const float value = ((float)quantized - (float)zero_point) * scale;
...
static bool linx_tile_operation_impl_dtype_supported(uint32_t op, uint32_t dtype)
{
    ...
    const uint32_t lowp = LINX_TILE_DTYPE_MASK(6u) | LINX_TILE_DTYPE_MASK(7u) |
                          LINX_TILE_DTYPE_MASK(8u) | LINX_TILE_DTYPE_MASK(9u) |
...
static bool linx_tile_operation_source_count_accepted(uint32_t impl_op,
                                                      unsigned source_count)
{
    if (impl_op == 0x084u) { /* TDEQUANT: scalar B.IOR (1) or legacy 3 tile */
        return source_count == 1u || source_count == 3u;
    }
    return impl_op != UINT32_MAX &&
           source_count ==
               (unsigned)linx_tile_operation_impl_source_arity(impl_op);
}
...
static bool linx_tile_operation(CPULinxState *env, unsigned dst_tile,
    ...
    uint32_t cols = 0;
    uint32_t physical_cols = 0;
    uint32_t tcvt_rows = 0;
    uint32_t tcvt_cols = 0;
    uint32_t tcvt_physical_cols = 0;
    uint32_t tcvt_physical_rows = 0;
    ...
    const uint32_t shape_elems = value_reduction && reduction_source_shape_valid
                                     ...
                                     : (uint32_t)(bytes64 /
                                           MAX(1u, elem_bytes));
    const bool tcvt_shape_valid =
        impl_op == 0x00du && has_src0 && src0_tile < LINX_TILE_SLOT_COUNT &&
        linx_tile_tcvt_descriptor(
            env->tile_reg_valid_cols[src0_tile],
            env->tile_reg_valid_rows[src0_tile],
            env->tile_reg_cols[src0_tile], env->tile_reg_rows[src0_tile],
            (uint32_t)bytes64, elem_bytes, env->lb[0] & 0xffffu,
            env->lb[1] & 0xffffu, env->lb[2] & 0xffffu,
            &tcvt_cols, &tcvt_rows, &tcvt_physical_cols,
            &tcvt_physical_rows);
    const bool operation_shape_valid = value_reduction
        ? reduction_source_shape_valid
        : impl_op == 0x00du
        ? tcvt_shape_valid
            ? (rows = tcvt_rows, cols = tcvt_cols,
               physical_cols = tcvt_physical_cols, true)
            : false
    ...
    if (!linx_tile_operation_selector_accepted(canonical_op) ||
        ...
        !linx_tile_operation_source_count_accepted(impl_op, source_count) ||
    ...
    if (elem_bytes == 8u && linx_tile_operation64_core_op(op)) {
        ...
        if (op == 0x00du ?
                !linx_tile_set_tcvt_shape(
                    env, dst_tile, src0_tile, (uint32_t)bytes64, 8u, true) :
            value_reduction ?
                !linx_tile_value_reduction_output_shape(
    ...
    if (op == 0x02bu) {
        ...
        /* TQUANT: affine-quantize a Local FP32 Tile into S8/U8.
         * The source element type is FP32 (from the BSTART DataType), the
         * destination type is carried by B.DATR (S8 or U8).  B.IOR RegSrc0
         * supplies the positive finite FP32 multiplier and RegSrc1 the
         * destination-typed integer zero point; omission selects 1.0 and 0. */
        if (!has_src0 || has_src1 || src0_elem_bytes != sizeof(uint32_t) ||
            (src0_dtype & 0x1fu) != 1u /* FP32 */ ||
            (dtype != 19u && dtype != 27u /* S8/U8 */) ||
            elem_bytes != 1u) {
            return false;
        }
        uint32_t scale_bits = 0x3f800000u;
        ...
        const uint32_t unsigned_mask = (1u << (elem_bytes * 8u)) - 1u;
        const int64_t zero_point = linx_tile_dtype_is_signed(dtype)
                                       ? (int64_t)linx_tile_sign_extend(zero_bits, elem_bytes)
                                       : (int64_t)(zero_bits & unsigned_mask);
        for (uint32_t r = 0; r < rows; r++) {
            for (uint32_t c = 0; c < cols; c++) {
                const uint32_t lane = r * physical_cols + c;
                ...
                const float input = linx_tile_word_as_f32(value);
                const float out = input * scale + (float)zero_point;
                const uint32_t result =
                    linx_tile_f32_as_dtype(env, out, dtype, elem_bytes);
                if (!linx_tile_set_elem(env, dst_tile, lane, elem_bytes,
                                        result)) {
                    return false;
                }
            }
        ...
        unsigned row_reg = 0;
        unsigned col_reg = 0;
        /* PTO v0.58 TINSERT two-source form: source0 is the persistent old
         * destination, source1 is the insertion tile.  Both are required. */
        ...
        /* TINSERT: destination carries the (old) destination tile as source0;
         * source1 is the insertion tile.  Copy the old destination first so
         * untouched cells retain their prior values, then overlay source1. */
        const uint32_t old_rows = env->tile_reg_valid_rows[src0_tile];
        const uint32_t old_cols = env->tile_reg_valid_cols[src0_tile];
        const uint32_t old_stride = env->tile_reg_cols[src0_tile];
        for (uint32_t r = 0; r < rows && r < old_rows; r++) {
            for (uint32_t c = 0; c < cols && c < old_cols; c++) {
                uint32_t value = 0;
                if (!linx_tile_get_elem(env, src0_tile, r * old_stride + c,
                                        elem_bytes, &value) ||
        ...
        const uint32_t ins_rows = env->tile_reg_valid_rows[src1_tile];
        const uint32_t ins_cols = env->tile_reg_valid_cols[src1_tile];
        const uint32_t ins_stride = env->tile_reg_cols[src1_tile];
        if (row_off + ins_rows > rows || col_off + ins_cols > cols) {
            return false;
        }
        for (uint32_t r = 0; r < ins_rows; r++) {
            for (uint32_t c = 0; c < ins_cols; c++) {
                uint32_t value = 0;
                if (!linx_tile_get_elem(env, src1_tile, r * ins_stride + c,
                                        elem_bytes, &value) ||
                    !linx_tile_set_elem(env, dst_tile,
        ...
        if (!linx_tile_operation_dequant(env, dst_tile, sources, source_count,
                                    rows, cols, physical_cols)) {
            return false;
        }
    } else if (op == 0x00du) {
    ...
    if (op == 0x00du ?
            !linx_tile_set_tcvt_shape(
                env, dst_tile, src0_tile, (uint32_t)bytes64,
                elem_bytes, true) :
        op == 0x01du ?
                           ...
                           uint64_t stride_bytes, bool has_stride)
{
    ...
    const uint32_t datr_layout = (env->tile_attr_raw >> 2) & 0x1fu;
    if (datr_layout >= 21u && datr_layout <= 23u) {
        const uint32_t valid_cols = env->lb[0] & 0xffffu;
        const uint32_t valid_rows = env->lb[1] & 0xffffu;
        const uint32_t cube_layout = datr_layout == 21u
                                         ? LINX_TILE_LAYOUT_CUBE_M32
                                     : datr_layout == 22u
                                         ? LINX_TILE_LAYOUT_CUBE_M16
                                         : LINX_TILE_LAYOUT_CUBE_N8;
        const uint64_t base = env->gpr[addr_reg];
        const bool packed = linx_tile_numeric_is_packed(dtype);
        const uint64_t dense_stride = packed
                                          ? (valid_cols + 1u) / 2u
                                          : (uint64_t)valid_cols * elem_bytes;
        const uint64_t effective_stride = has_stride ? stride_bytes
                                                      : dense_stride;
        if (!linx_tile_cube_descriptor_058(
                env, dst_tile, cube_layout, dtype, valid_rows, valid_cols,
                (uint32_t)bytes)) {
            helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
            return;
        }
        ...
        const uint32_t storage_bytes =
            env->tile_reg_cube_storage_bytes[dst_tile];
        const uint32_t physical_elements = linx_tile_numeric_is_packed(dtype)
                                               ? storage_bytes * 2u
                                               : storage_bytes / elem_bytes;
        for (uint32_t element = 0; element < physical_elements; element++) {
            uint64_t pad_raw;
            if (env->tile_attr_pad == 0u) {
                pad_raw = 0u;
            } else if (env->tile_attr_pad == 3u) {
                pad_raw = (UINT64_C(6364136223846793005) *
                           (element + 1u)) +
                          UINT64_C(1442695040888963407);
            } else {
                const double endpoint = env->tile_attr_pad == 1u
                                            ? DBL_MAX : -DBL_MAX;
                pad_raw = linx_tile_numeric_encode(dtype, endpoint, 0u, true);
            }
            if (linx_tile_numeric_is_packed(dtype)) {
                const uint8_t nibble = pad_raw & 0xfu;
                ((uint8_t *)env->tile_reg[dst_tile])[element / 2u] |=
                    nibble << ((element & 1u) * 4u);
            } else {
                memcpy((uint8_t *)env->tile_reg[dst_tile] +
                           (uint64_t)element * elem_bytes,
                       &pad_raw, elem_bytes);
            }
        }
        for (uint32_t row = 0; row < valid_rows; row++) {
            for (uint32_t col = 0; col < valid_cols; col++) {
                ...
                if (!linx_tile_cube_payload_index_058(
                        env, dst_tile, row, col, &index)) {
                    helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
                    return;
                }
                if (packed) {
                    value = linx_tile_mem_read64(
                        env, base + (uint64_t)row * effective_stride +
                                 col / 2u, 1u);
                    const uint8_t nibble =
                        linx_tile_numeric_nibble(value, col);
                    uint8_t *destination =
                        (uint8_t *)env->tile_reg[dst_tile] + index / 2u;
                    const unsigned shift = (index & 1u) * 4u;
                    *destination = (*destination & ~(0xfu << shift)) |
                                   (nibble << shift);
                } else {
                    value = linx_tile_mem_read64(
                        env, base + (uint64_t)row * effective_stride +
                                 (uint64_t)col * elem_bytes, elem_bytes);
                    memcpy((uint8_t *)env->tile_reg[dst_tile] +
                           (uint64_t)index * elem_bytes, &value, elem_bytes);
                }
            }
        }
        return;
                            ...
                            uint64_t stride_bytes, bool has_stride)
{
    ...
    const uint32_t datr_layout = (env->tile_attr_raw >> 2) & 0x1fu;
    if (datr_layout >= 24u && datr_layout <= 26u) {
        const uint32_t expected_layout = datr_layout == 24u
                                             ? LINX_TILE_LAYOUT_CUBE_M32
                                         : datr_layout == 25u
                                             ? LINX_TILE_LAYOUT_CUBE_M16
                                             : LINX_TILE_LAYOUT_CUBE_N8;
        ...
        const bool packed = linx_tile_numeric_is_packed(dtype);
        const uint64_t dense_stride = packed
            ? (env->tile_reg_valid_cols[src_tile] + 1u) / 2u
            : (uint64_t)env->tile_reg_valid_cols[src_tile] * elem_bytes;
        const uint64_t effective_stride = has_stride ? stride_bytes
                                                      : dense_stride;
        if (env->tile_reg_layout[src_tile] != expected_layout) {
            helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
            return;
        }
        for (uint32_t row = 0;
             row < env->tile_reg_valid_rows[src_tile]; row++) {
            for (uint32_t col = 0;
                 col < env->tile_reg_valid_cols[src_tile]; col++) {
                uint32_t index;
                uint64_t value = 0u;
                if (!linx_tile_cube_payload_index_058(
                        env, src_tile, row, col, &index)) {
                    helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
                    return;
                }
                if (packed) {
                    const uint8_t source =
                        ((uint8_t *)env->tile_reg[src_tile])[index / 2u];
                    const uint8_t nibble =
                        linx_tile_numeric_nibble(source, index);
                    const uint64_t address =
                        base + (uint64_t)row * effective_stride + col / 2u;
                    uint8_t destination = linx_tile_mem_read64(
                        env, address, 1u);
                    const unsigned shift = (col & 1u) * 4u;
                    destination = (destination & ~(0xfu << shift)) |
                                  (nibble << shift);
                    linx_tile_mem_write64(env, address, 1u, destination);
                } else {
                    memcpy(&value, (uint8_t *)env->tile_reg[src_tile] +
                           (uint64_t)index * elem_bytes, elem_bytes);
                    linx_tile_mem_write64(
                        env, base + (uint64_t)row * effective_stride +
    ...
    uint32_t gm_outer = 0;
    uint32_t gm_inner = 0;
    if (!linx_tile_tstore_descriptor_shape(
            env->tile_reg_valid_cols[src_tile],
            env->tile_reg_valid_rows[src_tile], env->tile_reg_cols[src_tile],
            env->tile_reg_rows[src_tile], (uint32_t)bytes, elem_bytes,
            &tr_outer, &tr_inner, &gm_outer, &gm_inner)) {
        helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
        return;
    }
...
static bool linx_tile_source_size_code(const CPULinxState *env,
                                      unsigned tile, unsigned *size_code_out)
{
    if (tile >= LINX_TILE_SLOT_COUNT) {
        return false;
    }
    ...
    uint32_t code = 0;
    uint32_t probe = 1u << 4u; /* 16 bytes minimum */
    while (probe < bytes && code < 60u) {
        probe <<= 1u;
        ++code;
    }
    if (probe != bytes) {
                                     ...
                                     unsigned size_code)
{
    ...
    const uint32_t active = valid_row * valid_col;
    const unsigned off_bytes =
        linx_tile_offset_elem_bytes(env, offset_tile, active);
    if (off_bytes == 0) {
        helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
        return;
    }
                                      ...
                                      unsigned size_code)
{
    uint32_t row = 0, col = 0, valid_row = 0, valid_col = 0, lane_count = 0;
    unsigned elem_bytes = 0;
    /* Source-only stores accept TSize=000 and derive capacity from the source
     * descriptor per B.IOT.md; the encoded size_code then supplies lane count. */
    if (size_code == 0u &&
        !linx_tile_source_size_code(env, data_tile, &size_code)) {
        helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
        return;
    }
    ...
    const uint32_t active = valid_row * valid_col;
    const unsigned off_bytes = env->tile_reg_elem_bytes[offset_tile];
    if (off_bytes == 0u || off_bytes > 8u ||
        env->tile_reg_bytes[offset_tile] < active * off_bytes ||
        env->tile_reg_bytes[data_tile] < active * elem_bytes) {
        helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
        return;
    }
    const bool signed_index =
        linx_tile_dtype_is_signed(env->tile_reg_dtype[offset_tile]);
    const uint64_t base = env->gpr[addr_reg];
    for (uint32_t r = 0; r < row; r++) {
        for (uint32_t c = 0; c < col; c++) {
            const uint32_t lane = r * col + c;
            ...
            if (!linx_tile_get_elem64(env, offset_tile, lane, off_bytes,
                                      &raw_off) ||
                                  ...
                                  unsigned size_code)
{
    ...
    const unsigned off_bytes =
        linx_tile_offset_elem_bytes(env, offset_tile, active);
    if (off_bytes == 0 ||
        env->tile_reg_bytes[expected_tile] < active * elem_bytes ||
        env->tile_reg_bytes[desired_tile] < active * elem_bytes) {
        helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
        return;
    }
    memset(env->tile_reg[dst_tile], 0, LINX_TILE_MAX_BYTES);
...
static uint32_t linx_tile_cube_accumulator_arch_dtype(
    const CPULinxState *env, bool mx)
{
    const uint8_t acc = mx ? LINX_TILE_ACC_FP32
                           : linx_tile_numeric_acc_dtype(env->tile_dtype);
    return acc == LINX_TILE_ACC_S64 ? 17u
           : acc == LINX_TILE_ACC_U64 ? 25u : acc;
}
                                              ...
                                              bool mx)
{
    ...
    const uint8_t acc_dtype = mx ? LINX_TILE_ACC_FP32
                                 : linx_tile_numeric_acc_dtype(env->tile_dtype);
    const uint32_t source_dtype = acc_dtype == LINX_TILE_ACC_S64 ? 17u
                                  : acc_dtype == LINX_TILE_ACC_U64 ? 25u
                                                                  : acc_dtype;
    const unsigned source_bytes = linx_tile_dtype_elem_bytes(source_dtype);
    ...
    for (unsigned row = 0; row < rows; row++) {
        for (unsigned col = 0; col < dims.n; col++) {
            ...
            } else if (acc_dtype == LINX_TILE_ACC_S64) {
                const int64_t widened = (int32_t)raw;
                memcpy(acc + output_index * 8u, &widened, 8u);
            } else if (acc_dtype == LINX_TILE_ACC_U64) {
                const uint64_t widened = (uint32_t)raw;
                memcpy(acc + output_index * 8u, &widened, 8u);
            } else {
                memcpy(acc + output_index * 8u, &raw, 8u);
            }
        }
    }
                                       ...
                                       uint64_t *span_bytes_out)
{
    ...
    if (source >= LINX_TILE_SLOT_COUNT || source_elem_bytes == 0u ||
        ...
        (rows == 1u ? env->tile_reg_capacity[source] < row_bytes
                               ...
                               unsigned addr_reg, unsigned size_code)
{
    ...
    if (record.elem_bytes == 0u || record.elem_bytes == 8u ||
        record.cols == 0u ||
        (rows > 1u && destination_stride < row_bytes) ||
    ...
    for (uint64_t row = 0; row < rows; row++) {
        for (uint64_t byte = 0; byte < row_bytes; byte++) {
            destination_data[(rows == 1u ? byte :
                              row * destination_stride + byte)] =
                                            ...
                                            unsigned *size_code_out)
{
    ...
    const unsigned size_class = (env->tile_shared_binder[0] >> 12) & 0xfu;

    if (size_class == 0u || size_class > 12u) {
        return false;
    }
    /* The wire SizeCode maps 1 -> 128 B. Internal allocation codes store
     * log2(bytes)-4, so every B.IOS destination uses the same wire+2 map as
     * B.IOT. */
...
{
    return env->tile_shared_binder[0] & 0x3fu;
}

static uint64_t linx_shared_capacity_in_use_locked(
    const LinxCore4State *core4, const LinxSharedTileVersion *exclude)
{
    uint64_t total = 0;
    for (unsigned tile = 0; tile < LINX_SHARED_TILE_COUNT; tile++) {
        const LinxSharedTileVersion *shared = &core4->shared_tile[tile];
        if (shared != exclude) {
            total += shared->allocated_bytes;
        }
    }
    return total;
}
...
static inline unsigned linx_tile_shared_size_code(const CPULinxState *env)
{
    return (env->tile_shared_binder[0] >> 12) & 0xfu;
}
...
static bool linx_tile_shared_tstore_legal(CPULinxState *env)
{
    ...
    bool valid = shared->allocation_mask != 0u &&
                 ...
                 shared->cols >= shared->valid_cols && shared->cols != 0u &&
                 shared->rows != 0u &&
...
static bool linx_tile_shared_tstore_commit(CPULinxState *env)
{
    ...
    for (uint32_t row = 0; row < snapshot.valid_rows; row++) {
        for (uint32_t col = 0; col < snapshot.valid_cols; col++) {
            ...
            const unsigned region = (unsigned)((source * 4u) /
                                                snapshot.per_pe_capacity);
...
static bool linx_tile_cube_publish_explicit_output(
    ...
    unsigned size_code)
{
    unsigned output_indices[3];
    unsigned outputs[3];
    unsigned sources[LINX_TILE_MAX_IOT * 2];
    unsigned source_count = 0u;
    unsigned output_count = 0u;
    unsigned mathematical_sources;
    unsigned ordinal;
    unsigned row_max_input = UINT_MAX;
    unsigned quant_tile = UINT_MAX;
    unsigned relu_tile = UINT_MAX;
    ...
    if (env->tile_shared_binder_count != 0u) {
        if (env->tile_shared_binder_count > mathematical_sources) {
            return false;
        }
        mathematical_sources -= env->tile_shared_binder_count;
    }
    ordinal = mathematical_sources;
    if (((env->tile_fpatr_raw >> 18) & 1u) != 0u &&
        ((env->tile_fpatr_raw >> 16) & 1u) != 0u) {
        row_max_input = sources[ordinal++];
    }
    if (linx_tile_fpatr_mode_uses_vector_parameter_058(mode)) {
        quant_tile = sources[ordinal++];
    }
    if (relu == 3u) {
        relu_tile = sources[ordinal++];
    }
    ...
    for (unsigned i = 0; i < env->tile_iot_count; i++) {
        ...
        if (output_count >= ARRAY_SIZE(outputs) ||
            !linx_tile_get_bound_output(env, i, &outputs[output_count])) {
            return false;
        }
    ...
    if (output_count == 0u ||
        !linx_tile_accumulator_convert_with_aux_058(
            env, outputs[0], size_code, quant_tile, relu_tile) ||
        !linx_tile_cube_reduction_outputs_with_input_058(
            env, row_max ? outputs[1] : UINT_MAX,
            group_max ? outputs[1u + row_max] : UINT_MAX,
            row_max_input)) {
        return false;
    }
    ...
    unsigned *size_code_out)
{
    ...
    if (env->tile_iot_count == 0u ||
        ...
        !linx_tile_operation_source_count_accepted(
            linx_tile_operation_impl_selector(op), *source_count_out)) {
        return false;
    }
...
typedef struct LinxTileCubeLocalOperands {
    unsigned accumulator;
    unsigned left;
    unsigned left_scale;
    unsigned right;
    unsigned right_scale;
    unsigned bias;
    unsigned mathematical_count;
    bool mx;
    bool accumulate;
    bool with_bias;
} LinxTileCubeLocalOperands;

static unsigned linx_tile_cube_auxiliary_source_count(
    const CPULinxState *env)
{
    const unsigned mode = (env->tile_fpatr_raw >> 26) & 0x3fu;
    const unsigned relu = (env->tile_fpatr_raw >> 23) & 0x7u;
    return ((((env->tile_fpatr_raw >> 18) & 1u) != 0u &&
             ((env->tile_fpatr_raw >> 16) & 1u) != 0u) ? 1u : 0u) +
           linx_tile_fpatr_mode_uses_vector_parameter_058(mode) +
           (relu == 3u);
}

static bool linx_tile_cube_resolve_local_operands(
    const CPULinxState *env, const unsigned *sources, unsigned source_count,
    LinxTileCubeLocalOperands *operands)
{
    const uint32_t func = env->tile_func & 0x1fu;
    const unsigned auxiliary_count =
        linx_tile_cube_auxiliary_source_count(env);
    unsigned cursor = 0u;
    unsigned mathematical_count;

    if (operands == NULL || source_count < auxiliary_count) {
        return false;
    }
    *operands = (LinxTileCubeLocalOperands) {
        .accumulator = UINT_MAX,
        .left = UINT_MAX,
        .left_scale = UINT_MAX,
        .right = UINT_MAX,
        .right_scale = UINT_MAX,
        .bias = UINT_MAX,
    };
    switch (func) {
    case LINX_CUBE_TMATMUL:
    ...
    case LINX_CUBE_TMATMUL_BIAS:
    case LINX_CUBE_TGEMV_BIAS:
        operands->with_bias = true;
        break;
    case LINX_CUBE_TMATMUL_ACC:
    case LINX_CUBE_TGEMV_ACC:
        operands->accumulate = true;
        break;
    case LINX_CUBE_TMATMUL_MX:
    case LINX_CUBE_TGEMV_MX:
        operands->mx = true;
    ...
    mathematical_count = source_count - auxiliary_count;
    if (operands->accumulate) {
        if (cursor >= mathematical_count) {
            return false;
        }
        operands->accumulator = sources[cursor++];
    }
    if (cursor >= mathematical_count) {
        return false;
    }
    operands->left = sources[cursor++];
    const uint32_t left_dtype =
        env->tile_reg_dtype[operands->left] & 31u;
    if (operands->mx &&
        linx_tile_numeric_mx_requires_scale(left_dtype)) {
        if (cursor >= mathematical_count) {
            return false;
        }
        operands->left_scale = sources[cursor++];
    }
    if (cursor >= mathematical_count) {
        return false;
    }
    operands->right = sources[cursor++];
    const uint32_t right_dtype =
        env->tile_reg_dtype[operands->right] & 31u;
    if (operands->mx &&
        linx_tile_numeric_mx_requires_scale(right_dtype)) {
        if (cursor >= mathematical_count) {
            return false;
        }
        operands->right_scale = sources[cursor++];
    }
    if (operands->with_bias) {
        if (cursor >= mathematical_count) {
            return false;
        }
        operands->bias = sources[cursor++];
    }
    if (cursor != mathematical_count) {
        return false;
    }
    operands->mathematical_count = mathematical_count;
    return true;
}
    ...
    unsigned sources[LINX_TILE_MAX_IOT * 2], unsigned *size_code_out)
{
    ...
    const bool row_max = ((env->tile_fpatr_raw >> 18) & 1u) != 0u;
    const bool group_max = ((env->tile_fpatr_raw >> 17) & 1u) != 0u;
    const unsigned expected_outputs = 1u + row_max + group_max;
    if (required_sources != UINT_MAX) {
        required_sources += linx_tile_cube_auxiliary_source_count(env);
    }
    ...
    for (unsigned i = 0; i < env->tile_iot_count; i++) {
        ...
        unsigned output;
        if ((final_desc && d.last == 0u) || (!final_desc && d.last != 0u)) {
            return false;
        }
        if (env->tile_iot_output_valid[i]) {
            if (!d.has_size ||
    ...
    return output_count == expected_outputs;
}

static bool linx_tile_cube_aux_source_legal(
    const CPULinxState *env, unsigned tile, unsigned rows, unsigned cols,
    uint32_t dtype)
{
    return tile < LINX_TILE_SLOT_COUNT &&
           env->tile_reg_layout[tile] == LINX_TILE_LAYOUT_ND &&
           linx_tile_cube_operand_legal(env, tile, dtype, rows, cols);
}

static bool linx_tile_cube_postprocess_sources_legal(
    const CPULinxState *env, const unsigned *sources,
    unsigned mathematical_sources)
{
    ...
    const uint32_t acc_arch_dtype = acc_dtype == LINX_TILE_ACC_S64 ? 17u
                                    : acc_dtype == LINX_TILE_ACC_U64 ? 25u
                                                                    : acc_dtype;
    unsigned ordinal = mathematical_sources;
    unsigned ior_count = 0u;
    unsigned reg;

    if (mode != 0u &&
        ((linx_tile_fpatr_mode_uses_s32_accumulator_058(mode)
              ? acc_dtype != LINX_TILE_ACC_S64
              : acc_dtype != LINX_TILE_ACC_FP32))) {
        return false;
    }
    if (!linx_tile_fpatr_datr_legal_058(
            mode, (env->tile_attr_raw >> 25) & 7u,
            ((env->tile_attr_raw >> 28) & 1u) != 0u)) {
        return false;
    }
    if (row_max && row_init) {
        if (!linx_tile_cube_aux_source_legal(
                env, sources[ordinal++], dims.m, 1u, acc_arch_dtype)) {
            return false;
        }
    }
    if (linx_tile_fpatr_mode_uses_vector_parameter_058(mode)) {
        const unsigned quant_tile = sources[ordinal++];
        if (!linx_tile_cube_aux_source_legal(
                env, quant_tile, 1u, dims.n, 24u)) {
            return false;
        }
        for (unsigned col = 0; col < dims.n; col++) {
            uint64_t value;
            if (!linx_tile_get_elem64(env, quant_tile, col, 8u, &value) ||
                !linx_tile_fpatr_quant_parameter_legal_058(mode, value)) {
                return false;
            }
        }
    }
    if (relu == 3u) {
        const unsigned relu_tile = sources[ordinal++];
        if (!linx_tile_cube_aux_source_legal(
                env, relu_tile, 1u, dims.n, 24u)) {
            return false;
        }
        for (unsigned col = 0; col < dims.n; col++) {
            uint64_t value;
            if (!linx_tile_get_elem64(env, relu_tile, col, 8u, &value) ||
                !linx_tile_fpatr_relu_parameter_legal_058(value)) {
                return false;
            }
        }
    }
    while (linx_tile_resolve_ior(env, ior_count, &reg)) {
        ior_count++;
    }
    const unsigned expected_ior =
        linx_tile_fpatr_mode_uses_scalar_parameter_058(mode) + (relu == 2u);
    if (ior_count != expected_ior) {
        return false;
    }
    if (linx_tile_fpatr_mode_uses_scalar_parameter_058(mode)) {
        if (!linx_tile_resolve_ior(env, 0u, &reg) ||
            !linx_tile_fpatr_quant_parameter_legal_058(mode,
                                                        env->gpr[reg])) {
            return false;
        }
    }
    if (relu == 2u) {
        const unsigned slot =
            linx_tile_fpatr_mode_uses_scalar_parameter_058(mode) ? 1u : 0u;
        if (!linx_tile_resolve_ior(env, slot, &reg) ||
            !linx_tile_fpatr_relu_parameter_legal_058(env->gpr[reg])) {
            return false;
        }
    }
    return true;
                                         ...
                                         LinxTLSUTransferDir dir)
{
    ...
    if (dir == LINX_TLSU_GM_TO_TR && datr_layout >= 21u &&
        datr_layout <= 23u) {
        const uint32_t dtype = linx_tile_effective_dtype(env);
        return ((env->tile_attr_raw >> 7) & 0x1fu) == 31u &&
               env->lb[0] != 0u && env->lb[1] != 0u && elem_bytes < 8u &&
               dtype != 14u && bytes >= LINX_TILE_CELL_BYTES;
    }
    const LinxTileFormatDesc fmt =
...
/* B.IOS SizeCode 1..12 maps to internal allocation codes 3..14. */
static bool linx_tile_shared_transfer_preflight(
    const CPULinxState *env, unsigned size_code)
{
    if (size_code < 3u || size_code > 14u) {
        return false;
    }
    const uint64_t bytes = 1ull << (size_code + 4u);
    if (bytes > LINX_SHARED_TILE_MAX_BYTES) {
        return false;
    }
    const unsigned elem_bytes =
        linx_tile_dtype_elem_bytes(linx_tile_effective_dtype(env));
...
static bool linx_tile_tstore_preflight(const CPULinxState *env,
                                       ...
                                       unsigned size_code)
{
    if (source >= LINX_TILE_SLOT_COUNT ||
    ...
    if (datr_layout >= 24u && datr_layout <= 26u) {
        const uint32_t expected = datr_layout == 24u
                                      ? LINX_TILE_LAYOUT_CUBE_M32
                                  : datr_layout == 25u
                                      ? LINX_TILE_LAYOUT_CUBE_M16
                                      : LINX_TILE_LAYOUT_CUBE_N8;
        return ((env->tile_attr_raw >> 7) & 0x1fu) == 31u &&
               elem_bytes != 0u && env->tile_reg_bytes[source] == bytes &&
               env->tile_reg_layout[source] == expected &&
               env->tile_reg_dtype[source] == (dtype & 0x1fu);
    }
    return fmt.valid && elem_bytes != 0u &&
           env->tile_reg_bytes[source] == bytes &&
           env->tile_reg_dtype[source] == (dtype & 0x1fu) &&
           env->tile_reg_elem_bytes[source] == elem_bytes &&
           linx_tile_tstore_descriptor_shape(
               env->tile_reg_valid_cols[source],
               env->tile_reg_valid_rows[source], env->tile_reg_cols[source],
               env->tile_reg_rows[source], (uint32_t)bytes, elem_bytes,
               &tr_outer, &tr_inner, &gm_outer, &gm_inner) &&
           linx_tile_layout_shape_valid(fmt.src, tr_outer, tr_inner,
                                        elem_bytes);
}
    ...
    uint8_t *planned_carrier_valid, uint8_t *planned_carrier)
{
    ...
    case LINX_TLSU_MGATHER_CAS: {
        ...
        if ((env->tile_func & 0x1f) == LINX_TLSU_MSCATTER ||
            (env->tile_func & 0x1f) == LINX_TLSU_MSCATTER_MASK) {
            if (env->tile_reg_valid_rows[sources[0]] !=
        ...
        const bool source_only_scatter =
            (env->tile_func & 0x1f) == LINX_TLSU_MSCATTER ||
            (env->tile_func & 0x1f) == LINX_TLSU_MSCATTER_MASK;
        for (unsigned i = 0; i < env->tile_iot_count; i++) {
            ...
            /* Source-only scatter accepts TSize=000 and derives capacity from
             * the source descriptor per B.IOT.md. */
            if ((!source_only_scatter || size_code != 0u) &&
                !linx_tile_size_code_valid(size_code)) {
                return false;
            }
            if (env->tile_iot_output_valid[i]) {
...
{
    bool duplicate = false;
    bool prior_iot_sources_only = true;
    const uint8_t mask = (shared >> 8) & 0xfu;
    ...
    for (unsigned i = 0; i < env->tile_shared_binder_count; i++) {
        if (((env->tile_shared_binder[i] >> 8) & 0xfu) != mask) {
            helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
            return;
        }
    }
...
void HELPER(linx_tile_append_iot)(CPULinxState *env, uint64_t packed)
{
    ...
    if (desc.has_size && env->tile_iot_count != 0u) {
        const LinxTileIOTDesc prev = linx_tile_decode_iot(
            env->tile_iot_desc[env->tile_iot_count - 1u]);
        split_size_completion = !prev.has_size && prev.src0 == desc.src0 &&
            prev.src1 == desc.src1 && prev.dst == desc.dst &&
            prev.last == desc.last && prev.flags == desc.flags &&
            (desc.reg == 0u || prev.reg == desc.reg);
    }
    if ((!split_size_completion &&
         env->tile_iot_count >= LINX_TILE_MAX_IOT) ||
        (!split_size_completion && env->tile_iot_count != 0u &&
         linx_tile_decode_iot(
             env->tile_iot_desc[env->tile_iot_count - 1u]).last != 0u)) {
        helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
        return;
    }
    for (unsigned i = 0; i < env->tile_iot_count; i++) {
        if (linx_tile_decode_iot(env->tile_iot_desc[i]).reg != desc.reg) {
            helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
            return;
        }
    }
    for (unsigned i = 0; i < env->tile_shared_binder_count; i++) {
        if (((env->tile_shared_binder[i] >> 8) & 0xfu) != desc.reg) {
            helper_raise_exception(env, LINX_EXCP_ILLEGAL_INST);
            return;
        }
    }
    ...
    if (desc.has_size && (vector_block || tile_output)) {
        ...
        uint32_t cube_valid_cols = 0u;
        uint32_t cube_valid_rows = 0u;
        uint32_t cube_cols = 0u;
        uint32_t cube_rows = 0u;
        unsigned cube_output_ordinal = 0u;

        for (unsigned planned = 0; planned < index; planned++) {
            cube_output_ordinal += env->tile_iot_output_valid[planned] != 0u;
        }
        ...
        if (env->blocktype == LINX_BLOCK_CUBE &&
            !linx_tile_cube_output_descriptor_058(
                env, cube_output_ordinal, (uint32_t)bytes64, &output_dtype,
                &cube_valid_cols, &cube_valid_rows, &cube_cols, &cube_rows)) {
        ...
        const bool conversion = operation_impl == 0x00du;
        const bool transpose = operation_impl == 0x01du;
        const bool shape_valid = (conversion || transpose || value_reduction) &&
                                 (src_valid & 1u) == 0u
            ? false
            : conversion
            ? linx_tile_set_tcvt_shape(
            ...
            : env->blocktype == LINX_BLOCK_CUBE
                ? cube_valid_cols != 0u && cube_valid_rows != 0u &&
                      cube_cols != 0u && cube_rows != 0u
...
static bool linx_tile_preflight_cube(const CPULinxState *env)
{
    ...
    for (unsigned i = 0; i < env->tile_iot_count; i++) {
        ...
        if (i == 0u) {
            output = d.reg;
        } else if (d.reg != output) {
            return false;
        }
    }
    for (unsigned i = 0; i < env->tile_shared_binder_count; i++) {
        if (((env->tile_shared_binder[i] >> 8) & 0xfu) != output) {
            return false;
        }
    }
    ...
    const bool primary_ok = linx_tile_cube_primary_legal_058(
        env, operands.left, operands.right, operands.mx, false);
    if (!primary_ok) {
        return false;
    }
    if (operands.accumulate &&
    ...
    if (operands.with_bias &&
        !linx_tile_cube_bias_legal_for(
            env, operands.bias,
            linx_tile_cube_accumulator_arch_dtype(env, operands.mx))) {
        return false;
    }
    return linx_tile_cube_postprocess_sources_legal(
        env, sources, operands.mathematical_count);
}
    ...
    unsigned *shared_count_out, unsigned *dst_out, unsigned *size_code_out)
{
    ...
    const bool sources_legal = linx_tile_collect_cube_sources(
        env, required_sources, sources, &size_code);
    const bool output_legal = env->tile_iot_count != 0u &&
    ...
    if (env->tile_iot_count > 1u &&
        !linx_tile_get_bound_output(env, 1u, &destination)) {
        return false;
    }
    if (env->tile_iot_count > 1u) {
        dst = destination;
    ...
    qemu_mutex_lock(&cpu->core4->lock);
    valid = true;
    for (unsigned binder = 0; valid && binder < shared_count; binder++) {
        const uint32_t packed = env->tile_shared_binder[binder];
        const unsigned shared_id = packed & 0x3fu;
        const uint8_t pe_mask = (packed >> 8) & 0xfu;
...
typedef struct LinxTileRegSnapshot {
    ...
    uint16_t cube_cell_count;
    uint32_t cube_storage_bytes;
    uint16_t valid_cols;
    uint16_t valid_rows;
    uint16_t cols;
    uint16_t rows;
                                   ...
                                   LinxTileRegSnapshot *snapshot)
{
    ...
    snapshot->cube_cell_count = env->tile_reg_cube_cell_count[tile];
    snapshot->cube_storage_bytes = env->tile_reg_cube_storage_bytes[tile];
                                  ...
                                  const LinxTileRegSnapshot *snapshot)
{
    ...
    env->tile_reg_cube_cell_count[tile] = snapshot->cube_cell_count;
    env->tile_reg_cube_storage_bytes[tile] = snapshot->cube_storage_bytes;
    ...
    uint8_t *acc_sources_valid, uint8_t acc_src0, uint8_t acc_src1)
{
    ...
    for (unsigned i = 0; i < env->tile_iot_count; i++) {
        ...
        if (env->blocktype == LINX_BLOCK_CUBE &&
            !linx_tile_cube_output_descriptor_058(
                env, (cube_ordinal = cube_output_ordinal++), bytes,
                &output_dtype,
                &cube_valid_cols, &cube_valid_rows, &cube_cols, &cube_rows)) {
            return false;
        }
        ...
        const bool conversion = operation_impl == 0x00du;
        const bool transpose = operation_impl == 0x01du;
        unsigned shape_source = 0u;
        const bool shape_source_valid =
            (!conversion && !transpose && !value_reduction) ||
            linx_tile_get_bound_source(env, i, 0u, &shape_source);
        if (!shape_source_valid || (conversion ?
                !linx_tile_set_tcvt_shape(
                    env, dst_tile, shape_source, bytes, elem_bytes, true) :
            transpose ?
                !linx_tile_set_transpose_shape(
                    env, dst_tile, shape_source, bytes, elem_bytes, true) :
                value_reduction ?
                ...
                !linx_tile_set_shape(
                    ...
                    bytes / elem_bytes, 1u) :
                env->blocktype == LINX_BLOCK_CUBE && cube_ordinal == 0u ?
                ({
                    unsigned cube_sources[LINX_TILE_MAX_IOT * 2];
                    unsigned cube_source_count = 0u;
                    const uint32_t cube_func = env->tile_func & 0x1fu;
                    const bool cube_acc = cube_func == LINX_CUBE_TMATMUL_ACC ||
                        cube_func == LINX_CUBE_TMATMUL_MX_ACC ||
                        cube_func == LINX_CUBE_TGEMV_ACC ||
                        cube_func == LINX_CUBE_TGEMV_MX_ACC;
                    const LinxTileCubeDimensions cube_dims =
                        linx_tile_cube_dimensions_058(env);
                    const bool collected = linx_tile_collect_sources(
                        env, cube_sources, &cube_source_count);
                    const uint32_t output_layout =
                        cube_source_count > cube_acc
                            ? env->tile_reg_layout[
                                  cube_sources[cube_acc ? 1u : 0u]]
                            : cube_dims.m <= 16u
                                ? LINX_TILE_LAYOUT_CUBE_M16
                                : LINX_TILE_LAYOUT_CUBE_M32;
                    !collected ||
...
static void linx_tile_group_clear_collective_locked(LinxCore4State *core4)
{
    ...
    core4->collective_shared_count = 0u;
...
static bool linx_tile_shared_tmov_local_to_shared(
    CPULinxState *env, uint16_t planned_live[LINX_TILE_HAND_COUNT],
    uint8_t *carrier_valid, uint8_t *carrier)
{
    LinxCPU *cpu = env_archcpu(env);
    ...
    if (valid && shared->allocation_mask == 0u &&
        linx_shared_capacity_in_use_locked(cpu->core4, shared) +
            shared_capacity > LINX_SHARED_TILE_MAX_BYTES) {
        valid = false;
    }
    if (valid && shared->allocation_mask == 0u) {
        shared->allocation_mask = mask;
        /* 0.58.4: SharedCoreSize is the complete object capacity; the
         * participation mask only selects fixed payload regions. */
        shared->per_pe_capacity = shared_capacity;
        shared->allocated_bytes = shared->per_pe_capacity;
        shared->dtype = dtype;
        shared->layout = env->tile_reg_layout[source];
...
static bool linx_tile_shared_tmov_shared_to_local(
    CPULinxState *env, uint16_t live[LINX_TILE_HAND_COUNT],
    uint16_t reserved[LINX_TILE_HAND_COUNT],
    uint8_t order[LINX_TILE_HAND_COUNT][LINX_TILE_HAND_DEPTH],
    uint8_t count_by_hand[LINX_TILE_HAND_COUNT])
...
static bool linx_tile_group_mma_commit(CPULinxState *env, uint64_t resume_pc)
{
    ...
    if (!linx_tile_group_cube_profile(env, &src_a, shared_id, &shared_count,
                                      &dst, &size_code)) {
        if (core4 != NULL) {
    ...
    if (core4->collective_arrived == 0u) {
        ...
        memcpy(core4->collective_shared_id, shared_id,
               sizeof(core4->collective_shared_id));
        ...
        valid = core4->collective_bpc == env->bpc &&
                ...
                memcmp(core4->collective_shared_id, shared_id,
                       sizeof(core4->collective_shared_id)) == 0 &&
                core4->collective_size_code == size_code &&
                core4->collective_m == dims.m &&
                core4->collective_n == dims.n &&
                core4->collective_k == dims.k &&
    ...
    /* Equivalent transactional state is carried as planned_live[LINX_CORE4_PE_COUNT]
     * and planned_count[i], 1u) in the single-descriptor publication path. */
                                        ...
                                        uint64_t resume_pc)
{
    ...
    for (unsigned i = 0; i < LINX_CORE4_PE_COUNT; i++) {
        ...
        destination_env->tile_reg_layout[output] = source_value->layout;
        destination_env->tile_reg_cube_k_repeat[output] =
            source_value->cube_k_repeat;
        destination_env->tile_reg_cube_n_repeat[output] =
            source_value->cube_n_repeat;
        destination_env->tile_reg_cube_cell_count[output] =
            source_value->cube_cell_count;
        destination_env->tile_reg_cube_storage_bytes[output] =
            source_value->cube_storage_bytes;
...
void HELPER(linx_tile_commit)(CPULinxState *env, uint64_t resume_pc)
{
    ...
    case LINX_BLOCK_TLSU:
        ...
        case LINX_TLSU_TLOAD: {
            if (shared_tload) {
                ...
                const uint32_t dtype = linx_tile_effective_dtype(env);
                const unsigned elem_bytes = linx_tile_dtype_elem_bytes(dtype);
                const uint32_t valid_cols = env->lb[0] != 0u ? env->lb[0] : 1u;
                const uint32_t valid_rows = env->lb[1] != 0u ? env->lb[1] : 1u;
                const uint32_t cols = env->lb[2] != 0u ? env->lb[2] : valid_cols;
                const uint32_t rows = elem_bytes != 0u && cols != 0u &&
                                      bytes % (elem_bytes * cols) == 0u
                                          ? bytes / (elem_bytes * cols) : 0u;
                const uint32_t layout = (env->tile_attr_raw >> 2) & 0x1fu;
                LinxSharedTileVersion *shared =
                    &cpu->core4->shared_tile[linx_tile_shared_id(env)];
                ...
                if (valid) {
                    const uint8_t allocation_mask = pe_mask;
                    ...
                    const bool new_version =
                        ...
                        shared->producer_bpc != env->bpc;
                    if (new_version) {
                        valid = linx_shared_capacity_in_use_locked(
                                    cpu->core4, shared) + bytes <=
                                LINX_SHARED_TILE_MAX_BYTES;
                        if (valid) {
                ...
                if (valid && shared->producer_bpc == env->bpc &&
                    ...
                    for (unsigned region = 0; region < LINX_CORE4_PE_COUNT;
                         region++) {
                        if ((pe_mask & (uint8_t)(1u << region)) == 0u ||
                            cpu->core4->cpu[region] == NULL) {
                            continue;
                        }
                        CPULinxState *peer = &cpu->core4->cpu[region]->env;
                        ...
                        for (uint32_t row = 0; row < valid_rows; row++) {
                            for (uint32_t col = 0; col < valid_cols; col++) {
                                ...
                                const unsigned destination_region =
    ...
    case LINX_BLOCK_CUBE: {
        ...
        valid = linx_tile_collect_sources(env, sources, &source_count) &&
                linx_tile_collect_cube_sources(
                    env, UINT_MAX, sources, &size_code) &&
                linx_tile_cube_resolve_local_operands(
                    env, sources, source_count, &operands);
        if (valid && operands.accumulate) {
            valid = linx_tile_cube_stage_accumulator(
                env, operands.accumulator, size_code, operands.mx);
        }
        if (valid) {
            valid = linx_tile_cube_compute(
                env, operands.left, operands.right,
                operands.left_scale, operands.right_scale,
                operands.bias, size_code, operands.mx,
                operands.with_bias, operands.accumulate);
        }
        if (valid) {
            valid = linx_tile_cube_publish_explicit_output(
                env, live, reserved, order, count_by_hand,
