...
/*
 * Scalar DIV/REM follow the SAIL reference conventions (non-trapping):
 *   divisor == 0        -> quotient 0, remainder == dividend;
 *   signed INT_MIN / -1 -> quotient and remainder both equal the dividend.
 * Raw host division raises SIGFPE on x86 for these inputs, so guard the
 * TCG division/remainder ops explicitly before emitting them.
 */
static void linx_emit_divrem(TCGv_i64 out, TCGv_i64 lhs, TCGv_i64 rhs,
                             bool is_div, bool is_signed, bool is_word)
{
                           ...
                           bool is_word)
{
    ...
    if (is_word) {
        ...
        linx_emit_divrem(out, l32, r32, is_div, is_signed, is_word);
